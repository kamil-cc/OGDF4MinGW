...
void FMMMLayout :: initialize_all_options()
{
    ...
    //setting options for different repulsice force calculation methods
    frGridQuotient(2); 
...
void FMMMLayout :: create_initial_placement (Graph& G,NodeArray<NodeAttributes>& A) 
{
 ...
 if (initialPlacementForces() == ipfUniformGrid) //set nodes to the midpoints of a  grid
   {//(uniform on a grid)
				  ...
				  <NodeAttributes> &A, NodeArray<DPoint>& F_rep)
{
  if(repulsiveForcesCalculation() == rfcExact)
     FR.make_initialisations(G,A,F_rep,boxlength,down_left_corner,frGridQuotient());
  else if(repulsiveForcesCalculation() == rfcGridApproximation)
     FR.make_initialisations(G,A,F_rep,boxlength,down_left_corner,frGridQuotient());
  else //(repulsiveForcesCalculation() == rfcNMM
    NM.make_initialisations(G,A,F_rep,boxlength,down_left_corner,
