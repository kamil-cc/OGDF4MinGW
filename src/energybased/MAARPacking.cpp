							    ...
							    bounding_rectangles_area)
{
 ...
 for(rect_item = R.begin(); rect_item.valid(); ++rect_item)
  {
    if (P.empty())
    {
     ...
     {
      B_F_item = find_Best_Fit_insert_position(rect_item,R,allow_tipping_over,P,
 ...
 export_new_rectangle_positions(P,row_of_rectangle,rectangle_order,R);
...
ListIterator<PackingRowInfo> MAARPacking :: find_Best_Fit_insert_position(
						...
						rect_item,List<Rectangle>& R,int 
						allow_tipping_over,List<PackingRowInfo>& 
						P,double aspect_ratio, double& 
						aspect_ratio_area,PQueue& 
						total_width_of_row)
...
inline Rectangle MAARPacking :: tipp_over(List<Rectangle>& R,ListIterator<Rectangle>
					  rect_item)
{
  Rectangle r = *rect_item;
