 ...
 * last checkin:
 *   $Author: klein $ 
 *   $Date: 2007-11-15 14:43:46 +0100 (Do, 15 Nov 2007) $ 
 ***************************************************************/
...
//layout given by AG
UniformGrid::UniformGrid(const GraphAttributes &AG) : m_layout(AG),
m_graph(AG.constGraph()),m_crossings(m_graph), m_cells(m_graph), m_CellSize(0.0), m_crossNum(0)
 {
	//cout<<"New grid \n";
...
UniformGrid::UniformGrid(const GraphAttributes &AG, const node v,
						 const DPoint& newPos) : m_CellSize(0.0),m_layout(AG),
m_graph(AG.constGraph()), m_crossings(m_graph), m_cells(m_graph), m_crossNum(0)
 {
#ifdef OGDF_DEBUG
	m_crossingTests = 0;
	m_maxEdgesPerCell = 0;
