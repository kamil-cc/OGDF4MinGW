 ...
 * last checkin:
 *   $Author: klein $ 
 *   $Date: 2007-11-15 14:07:28 +0100 (Do, 15 Nov 2007) $ 
 ***************************************************************/
...
namespace ogdf{
	...
	bool DavidsonHarel::testEnergyValue(double newVal)
	{
		...
		if(newVal > m_energy) {
			accepted = false;
			if (m_fineTune != tpFine) //only worse states allowed if coarse tuning
			{
...
#ifdef OGDF_DEBUG
		ListConstIterator<node> it_d;
		for(it_d = m_nonIsolatedNodes.begin(); it_d.valid(); ++it_d) {
			node v = *it_d;
			double x = AG.x(v);
			double y = AG.y(v);
			OGDF_ASSERT(x > minX && x < maxX && y > minY && y < maxY);
		}
#endif
	...
	void DavidsonHarel::call(GraphAttributes &AG, TuningParameter fineTune)
	{
		m_fineTune = fineTune;
		initParameters();

		//change the radius shrinking depending on tuning
		if (m_fineTune == tpCoarse) m_shrinkingFactor = 0.75;
		if (m_fineTune == tpFine)   m_shrinkingFactor = 0.92;
		if (m_fineTune == tpFree)   m_shrinkingFactor = m_shrinkFactor;
