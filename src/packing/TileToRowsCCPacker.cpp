...
template<class POINT>
class DecrIndexComparer : public Comparer<int>
{
	...
	double pageRatio)
{
	...
	//	if (bestRow < 0) {
	//		struct RowInfo &r = row[nRows++];
	//		r.m_boxes.pushBack(i);
	//		r.m_maxHeight = box[i].m_y;
	//		r.m_width = box[i].m_x;

	//	} else {
	//		struct RowInfo &r = row[bestRow];
	//		r.m_boxes.pushBack(i);
	...
	//double y = 0;  // sum of the heights of boxes 0,...,i-1
	//for(i = 0; i < nRows; ++i)
	//{
	//	const RowInfo &r = row[i];

	//	double x = 0;  // sum of the widths of the boxes to the left of box *it

	//	SListConstIterator<int> it;
	//	for(it = r.m_boxes.begin(); it.valid(); ++it)
	//	{
	//		offset[*it] = DPoint(x,y);
	//		x += box[*it].m_x;
	//	}
