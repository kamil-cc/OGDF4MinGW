 ...
 * last checkin:
 *   $Author: gutwenger $ 
 *   $Date: 2007-11-15 16:24:08 +0100 (Do, 15 Nov 2007) $ 
 ***************************************************************/
...
void EmbedderMinDepthPiTa::call(PlanRep& PG, adjEntry& adjExternal)
{
	...
	if (pBCTree->bcTree().numberOfNodes() != 1)
	{
		...
		forall_nodes(bT, pBCTree->bcTree())
		{
			...
			node cT = bT->firstAdj()->twin()->theNode();
			...
			if (SG.numberOfEdges() == 1)
			{
				...
				edge dummyEdge1 = PG.newEdge(PGnode1, dummyNodePG);
				edge dummyEdge2 = PG.newEdge(PGnode2, dummyNodePG);
			}
		}
	}
	delete pBCTree;
	...
	//***************************************************************************/
	node vT = rootBlockNode->firstAdj()->twin()->theNode();
	bcTreePG.clear();
	...
	{
		node nr = m_rootOfBlockCutfaceTree->firstAdj()->twin()->theNode();
	...
	{
		rmBCFT = m_rootOfBlockCutfaceTree->firstAdj()->twin()->theNode();

		//adjExternal = *((*(faces.get(nDG_to_fPG[m_blockCutfaceTree.original(m_blockCutfaceTree.cutVertex(m_rootOfBlockCutfaceTree->firstAdj()->twin()->theNode(), m_rootOfBlockCutfaceTree->firstAdj()->twin()->theNode()))]))).begin());
		//return;
	}
			...
			forall_faces(f, CE)
			{
				...
				if (numOfEntriesFromList == childrenOfKnot.size())
				{
					...
					for (ListIterator<node> it = childrenOfKnot.begin(); it.valid(); it++)
					{
						...
						{
							node otherNode = (*it_ae)->twin()->theNode();
...
void EmbedderMinDepthPiTa::embedBlocks(const node& bT, const node& cH)
{
	...
	node m_cH = cH;
	if (m_cH == 0)
		m_cH = pBCTree->cutVertex(bT->firstAdj()->twin()->theNode(), bT);
...
void EmbedderMinDepthPiTa::embedCutVertex(const node& vT, bool root /*= false*/)
{
	...
	node adj_bT = vT->firstAdj()->twin()->theNode();
	...
	forall_adj_edges(e_vT_bT, vT)
	{
		...
		if (Gamma_adjExt_nT[vTp] == 0)
		{
			...
			node twinG = nG_nT_to_nPG[bTp][Gamma_adjExt_nT[bTp]->twin()->theNode()];
			node twinG_vT = nPG_to_nG_nT[vTp][twinG];
			adjEntry ae;
			forall_adj(ae, nodeG_vT)
			{
				if (ae->twin()->theNode() == twinG_vT)
				{
	...
	if (root)
	{
		...
		forall_nodes(n, pBCTree->originalGraph())
		{
			...
			{
				node twinPGnode = nG_nT_to_nPG[vTp][(*it)->twin()->theNode()];
				adjEntry ae_n;
				forall_adj(ae_n, n)
				{
					if (ae_n->twin()->theNode() == twinPGnode)
					{
