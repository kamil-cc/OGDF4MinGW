	...
	for(i = 0; i < numCC; ++i)
	{
		...
		forall_edges(e,PG) 
		{
			...
			if (PG.typeOf(e) == Graph::generalization)
			{
				...
				if (ori)
				{
					OGDF_ASSERT(!(noCrossingEdge[ori]))
				}
#endif
				preferedEdges.pushBack(e);
		...
		//		forall_adj(adj, cent)
		//		{
		//			OGDF_ASSERT(PG.original(adj->twin()->theNode()))
			...
			while (itNode.valid())
			{
				...
				if (adjBoundary)
				{
					...
					do {
						...
						if (first)
						{
							maxx = minx = drawing.x(adjRunner->theNode());
							maxy = miny = drawing.y(adjRunner->theNode());
							first = false;
						}
						else
						{
						...
						OGDF_ASSERT((adjRunner->twin()->theNode()->degree() == 2) ||
							(adjRunner->twin()->theNode()->degree() == 4))
							//bend
							if (adjRunner->twin()->theNode()->degree() < 4) 
				...
				else 
				{
					...
					{
						node w = adjBoundary->twin()->theNode();
						if (first)
						{
							minx = maxx = drawing.x(PG.copy(w));
							miny = maxy = drawing.y(PG.copy(w));
							first = false;
						}
						else
						{
...
void PlanarizationLayout::assureDrawability(UMLGraph &UG)
{
	...
	forall_edges(e, G) {
		if (e->isSelfLoop())  
			THROW_PARAM(PreconditionViolatedException, pvcSelfLoop);
	}
	...
	if (!dfsGenTree(UG, m_fakedGens, m_fakeTree))
		THROW_PARAM(PreconditionViolatedException, pvcTreeHierarchies);
	...
	Layout& drawing)
{
	...
	do
	{
		...
		//anchor node
		OGDF_ASSERT(adjRun->twin()->theNode()->degree() == 4)
		...
		{
			//we search for the edge outside the node cage
			OGDF_ASSERT(adjURun->twin()->theNode()->degree() == 4)
