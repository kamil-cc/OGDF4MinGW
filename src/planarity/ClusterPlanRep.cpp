...
ClusterPlanRep::ClusterPlanRep(const ClusterGraphAttributes &acGraph, 
	...
	const SList<adjEntry> &crossedEdges)
{
	...
	for(it = chain(eOrig).begin(); it.valid(); ++it)
	{
		...
		node v1 = adjC1->twin()->theNode();
		node v2 = adjC2->twin()->theNode();
		...
		if (orV1 || orV2) //a dummy (crossing/boundary) and an original
		{
			...
			if (orC == dC) m_nodeClusterID[dummy] = orC->index();
				 ...
				 else THROW (AlgorithmFailureException);
			continue;
		}//if one original
