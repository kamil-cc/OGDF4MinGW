...
//#include <ogdf/basic/simple_graph_alg.h> // euler-upperbound of edges

namespace ogdf {


// returns false, if the euler-bound e<=3*n-6 is not satisfied
bool BoyerMyrvold::euler(const Graph& g) {
	// only valid, if graph is simple
	//if (!isSimpleUndirected(g)) return true; // unfortunately too slow, but necessary
	
	// number of isolated vertices
	node v;
	int isolated = 0;
	forall_nodes(v,g) {
		if (v->degree() == 0) isolated++;
	}
	
	// the upper bound of euler's formula for edges
	int n = g.numberOfNodes();
	if ((n-isolated > 2) && (g.numberOfEdges() > 3*(n-isolated)-6))
		return false;
...
bool BoyerMyrvold::planarDestructive(Graph& g) {
	...
	// euler bound is too slow due to possible parallel edges
	//if (!euler(g)) return false;
...
bool BoyerMyrvold::planar(const Graph& g) {
	...
	// euler bound is too slow due to the check on parallel edges
	//if (!euler(g)) return false;
					...
					bool avoidE2Minors)
{
	...
	if (embeddingGrade > BoyerMyrvoldPlanar::doFindZero ||
		...
		//extract.statisticsOutput(dummy);
		if (bundles) {
					...
					bool avoidE2Minors)
{
	...
	if (planar) {
		...
		cout << "\n";
		forall_nodes(v,g) {
			cout << "\nnode " << pBMP->m_dfi[h.copy(v)] << ": ";
			adjEntry adj;
			forall_adj(adj,v) {
				cout << pBMP->m_dfi[h.copy(adj->theEdge()->opposite(v))] << ",";
			}
		}
		*/
	}
...
// returns true, if constant graph g is planar, false otherwise. in addition, 
// h contains a planar embedding, if planar. if not planar, list
// contains a kuratowski subdivision.
// use this slower function, if g must not be changed.
// use embeddingGrade to bound the overall number of extracted kuratowski subdivisions;
