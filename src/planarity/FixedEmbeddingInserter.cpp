	...
	const EdgeArray<unsigned int> *edgeSubGraph)
{
...
#ifdef OGDF_DEBUG
		if(forbiddenEdgeOrig != 0) {
			SListIterator<adjEntry> it = crossed.begin();
			for(++it; it != crossed.rbegin(); ++it) {
				edge e = PG.original((*it)->theEdge());
				OGDF_ASSERT((*forbiddenEdgeOrig)[e] == false);
			}
		}
#endif
	...
	if(forbiddenEdgeOrig != 0) {
		edge e;
		forall_edges(e,G) {
			OGDF_ASSERT((*forbiddenEdgeOrig)[e] == false || PG.chain(e).size() == 1);
		}
	}
#endif
