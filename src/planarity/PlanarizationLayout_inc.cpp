    ...
    const EdgeArray<bool> &fixedEdges)
{
	try {
		...
		for(i = 0; i < numCC; ++i)
		{
			...
			bool unConnected = PG.makeTreeConnected(adjExternal);
			...
			if ((PG.numberOfEdges() > 0) || singleNode)
			{
				...
				while (itAdd.valid())
				{
					...
					else
					{
						...
						int count = 0;
						while ((adjExternal->theNode() == adjExternal->twin()->theNode()) &&
							(count < eNum))
						{
							adjExternal = adjExternal->faceCycleSucc();
							count++;
						...
						if (adjMerger != 0)
						{
							...
							while (runAdj != adjMerger)
							{
								...
								if (eUp)
								{
									...
									eBends.pushBack(DPoint(drawing.x(adjUp->twin()->theNode()),
									drawing.y(adjUp->twin()->theNode())));
							...
							forall_adj(adjMerger, vMerger)
							{
								if (adjMerger->theEdge()->target() == vMerger)
								{
									...
									if (eOrig)
									{
										...
										{
											if (adjUp)
												umlGraph.bends(eOrig).pushBack(DPoint(drawing.x(adjUp->twin()->theNode()),
												drawing.y(adjUp->twin()->theNode())));
										}//else
									}//if
	...
	const NodeArray<bool> &fixedNodes)
{
	...
	while (!nodeQ.empty())
	{
		...
		{
			node testNode = adjE->twin()->theNode();
	...
	const NodeArray<bool> &fixedNodes)
{
	...
	while (it.valid())
	{
		...
		adjE = (*it)->firstAdj();
		do {
			if (fixedNodes[adjE->twin()->theNode()])
			{
				vDegree++;
				someFixedNode = adjE->twin()->theNode();
			}
