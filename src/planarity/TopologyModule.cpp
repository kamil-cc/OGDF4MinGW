		 ...
		 GraphAttributes &AG, adjEntry &adjExternal, bool setExternal, bool reuseAGEmbedding)
{
	...
	{
#ifdef OGDF_DEBUG
		((PlanRepInc&)PG).writeGML("c:\\temp\\ogdl\\Vorplanaris.gml", AG);
#endif
 		planarizeFromLayout(PG, AG);
		if (!PG.representsCombEmbedding())
			handleImprecision(PG);
#ifdef OGDF_DEBUG
		((PlanRepInc&)PG).writeGML("c:\\temp\\ogdl\\Nachplanaris.gml", AG);
#endif
	}
	...
	if(!PG.representsCombEmbedding())
	{
		...
		//wiedereingefuegt, da Absturz in Unittest mit Positions = (0,0)
		pm.planarEmbed(PG);
	}
...
//debug
#ifdef OGDF_DEBUG
	ofstream f("c:\\temp\\ogdl\\AdjSortCopy.txt");
	node v;
	adjEntry ae;
	forall_nodes(v, PG)
	{
		if (PG.original(v))
		{
			...
			forall_adj(ae, v)
			{ 
				node w = PG.original(ae->twin()->theNode());
				if (w)
					f << "  Eintrag: " << w->index() << "\n";
				else f <<"No original\n";
...
//we handle and correct cases were imprecision, e.g.
...
void TopologyModule::postProcess(PlanRep &PG)
{
	...
	if (m_options & opLoop)
	{
		...
		forall_nodes(v, PG)
		{
			...
			do
			{
				node w = adRun2->twin()->theNode();
				node u = adRun1->twin()->theNode();
				if ( (w->degree() == 4) && (w == u) &&
					(w != v) ) 
				{
							 ...
							 GraphAttributes &AG)
{
	...
	//--
	edge e = AG.constGraph().firstEdge();
	//--
	...
	if ( ((DIsLess(aVal1*yB1 - aVal2*xB1,  
		...
		&& 
		  ...
		  (DIsLess(bVal1*y2  - bVal2*x2,
		   bVal1*yB1 - bVal2*xB1)))  //kann von oben genommen werden
		)
	{
...
bool TopologyModule::checkFlipCrossing(PlanRep& PG, node v, bool flip)
{
	...
	if (PG.isCrossingType(v))
	{
		...
		node va1 = a1->twin()->theNode();
		node va2 = a2->twin()->theNode();
		node vb1 = b1->twin()->theNode();
		node vb2 = b2->twin()->theNode();
...
int PointComparer::compare(const ListIterator<EdgeLeg*> &ep1, const ListIterator<EdgeLeg*> &ep2)
{
	DPoint p1 = (*ep1)->m_xp;
	DPoint p2 = (*ep2)->m_xp;
