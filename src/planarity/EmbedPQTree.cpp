...
//	PQNodeKey<edge,indInfo*,bool>	    *nodeInfoPtr = 0;
	embedKey						    *nodeInfoPtr = 0; // dummy
	...
	if (addIndicator)
	{
		...
		{		
			cout << "Indicator added : " << v;// << endl; 
			cout << "Controlling !" << endl;
		}
		#endif
		indInfo *newInfo = OGDF_NEW indInfo(v);
//		nodeInfoPtr = OGDF_NEW PQNodeKey<edge,indInfo*,bool>(newInfo);
		nodeInfoPtr = OGDF_NEW embedKey(newInfo);
	...
	if (!leafKeys.empty() && leafKeys.front() == leafKeys.back())
	{
		...
		if (addIndicator)
		{
			...
			{
				OGDF_ASSERT(m_pertinentRoot->endmostChild())
		...
		
{
	...
	OGDF_ASSERT(beginSequence && endSequence) // No sequence found.
	...
	currentNode = beginSequence;
	OGDF_ASSERT(currentNode != endSequence)
...
PQNode<edge,indInfo*,bool>* EmbedPQTree::
...
{   
	PQNode<edge,indInfo*,bool>*  checkNode = 0; // dummy
//	PQNode<edge,indInfo*,bool>*  prevNode = 0;  // dummy

	Stack<PQNode<edge,indInfo*,bool>*> S;
//	Stack<PQNode<edge,indInfo*,bool>*> S2; // dummy
	S.push(nodePtr);
	...
	while (!S.empty())
	{
		...
		else
		{
			...
			{
				OGDF_ASSERT(checkNode->getEndmost(RIGHT))
			...
			{
				OGDF_ASSERT(checkNode->type() == Q_NODE)
...
// These keys include keys of direction indicators detected in the frontier.
//
// No directiuon is assigned to the direction indicators.
//
