...
#define OUTPUT_BEFOREEXTRACTION_FILE "../BeforeExtraction.gml"
				...
				const edge startExlude = NULL)
{
	...
	#ifdef STATISTICS
	if (stack.empty()) stat << "\n(no valid startedges found in DynamicBacktrack::init)";
	#endif
};
...
bool DynamicBacktrack::addNextPath(SListPure<edge>& list, node& endnode) {
	...
	/*
	#ifdef STATISTICS
	StackPure<adjEntry> copy = stack;
	stat << "\nstack of addNextPath: ";
	while (!copy.empty()) {
		adj = copy.pop();
		if (adj) stat << m_dfi[adj->twin()->theNode()] << ",";
			else stat << "NULL,";
	}
	#endif*/
	...
	while (!stack.empty()) {
		...
		if (adj==NULL) {
			...
			/*
			#ifdef STATISTICS
			stat << "\nbacktrack to node " << m_dfi[v] << flush;
			#endif
			*/
			continue;
		}
		...
		*/
		...
		forall_adj(adj,v) {
			...
			if ((m_flags[adj->theEdge()] & flag) && (m_parent[adj->twin()->theNode()]==NULL)) {
				/*
				#ifdef STATISTICS
				stat << "\nPush: NULL, " << m_dfi[adj->twin()->theNode()];
				#endif
				*/
				stack.push(NULL);
				stack.push(adj);
		...
		m_adjParent(bm.m_adjParent)
{
	...
	#ifdef STATISTICS
	stat << "\n------------------------------------------------------------";
	stat << "\nflip whole graph for ExtractKuratowskis: ";
	#endif
	
	// flip Graph and merge virtual with real nodes, if not already done
	bm.flipBicomp(1,-1,m_wasHere,true,true);
	#ifdef FILE_OUTPUT
	Output::dfsTreeOutput(bm,m_ga,OUTPUT_BEFOREEXTRACTION_FILE,true);
	#endif
	#ifdef OGDF_DEBUG
	if (!m_avoidE2Minors) {
		cout << "\nIsANewKuratowski-tests are not performed, ";
		cout << "due to possible multiple E2/AE2-minors";
	}
	#endif
};
...
int ExtractKuratowskis::whichKuratowski(
	...
	for (it = list.begin(); it.valid(); ++it) {
		...
		if (edgenumber[e] == 1) {
			#ifdef STATISTICS
			stat << "\nSome edges occur more than once! ";
			if (m_dfi.valid()) {
				stat << "(e.g. edge ";
				stat << m_dfi[e->source()] << "<->" << m_dfi[e->target()] << ")";
			}
			#endif
					...
					EdgeArray<int>& edgenumber)
{
	...
	forall_nodes(v,m_g) {
		if (nodenumber[v] > 4 || nodenumber[v] == 1) {
			#ifdef STATISTICS
			stat << "\nThere are nodes with degree higher than 4 or less than 2!";
			#endif
			return ExtractKuratowskis::none;
		}
		if (nodenumber[v]==3) {
	...
	if (degree3nodes == 6) {
		if (degree4nodes > 0) {
			#ifdef STATISTICS
			stat << "\nThere are additional degree 4-nodes besides a K3,3!";
			#endif
			return ExtractKuratowskis::none;
		}
		for (int i=0; i<6; ++i) {
				...
				if (edgenumber[e] > 0) { // not visited
					...
					if (K33Partition[i] != K33Partition[ii]) {
						...
						if (!K33Links[i][ii]) {
							...
							#ifdef STATISTICS
							stat << "\nThere exists more than one path between the nodes ";
							if (m_dfi.valid()) {
								stat << m_dfi[K33Nodes[i]] << " and " << m_dfi[v] << " ";
							}
							stat << "of different equivalence classes in the K3,3!";
							#endif
							return ExtractKuratowskis::none;
						}
					} else {
						#ifdef STATISTICS
						stat << "\nThere exists a path between the nodes ";
						if (m_dfi.valid()) {
							stat << m_dfi[K33Nodes[i]] << " and " << m_dfi[v] << " ";
						}
						stat << "of the same equivalence class in the K3,3!";
						#endif
						return ExtractKuratowskis::none;
					}
				}
		...
		if (paths==9) {
			#ifdef STATISTICS
			stat << " ...is a K3,3" << flush;
			#endif
			return ExtractKuratowskis::K33;
		} else {
			#ifdef STATISTICS
			stat << "\nThere are " << paths << " instead of 9 disjoint paths in K33!";
			#endif
			return ExtractKuratowskis::none;
		}
		...
		if (degree3nodes > 0) {
			#ifdef STATISTICS
			stat << "\nThere are additional degree 3-nodes besides a K5!";
			#endif
			return ExtractKuratowskis::none;
		}
		for (int i=0; i<5; ++i) {
		...
		if (paths==10) {
			...
			#ifdef STATISTICS
			stat << "\nThere are " << paths << " instead of 10 disjoint paths in K5!";
			#endif
			return ExtractKuratowskis::none;
		}
	} else {
		#ifdef STATISTICS
		stat << "\nThere don't exist 6 degree-3 nodes nor 5 degree-4 nodes!";
		#endif
		return ExtractKuratowskis::none;
	}
}
					...
					const SListPure<edge>& list2)
{
	...
	#ifdef STATISTICS
	if (!list1.empty() && it.valid() && list1.front() == *it)
		stat << "\ntruncate Edgelist at edges: ";
	#endif
	while (!list1.empty() && it.valid() && list1.front() == *it) {
		#ifdef STATISTICS
		stat << m_dfi[list1.front()->source()] << "<->" << m_dfi[list1.front()->target()] << ", ";
		#endif
		list1.popFront();
		++it;
	}
				...
				const SListPure<edge>& pathW)
{
	...
	#ifdef FILE_OUTPUT
	char buffer[255];
	std::sprintf(buffer,"%s%s",KURATOWSKI_FILE,"MinorA");
	Output::kuratowskiOutput(BMP,m_ga,buffer,k,info,pathX,pathY,pathW,A.edgeList);
	#endif
				...
				const SListPure<edge>& pathW)
{
	...
	for (itStart = externE.startnodes.begin(); itStart.valid(); ++itStart) {
		...
		if (!(*itPath).empty()) {
			...
			#ifdef STATISTICS
			stat << "\nMinorB: adjToLowestNodeBelow[" << m_dfi[externE.theNode];
			stat << "] = " << m_dfi[adj->theNode()];
			#endif
		...
		if (info.minorType & WInfo::A) {
			std::sprintf(buffer,"%s%s",KURATOWSKI_FILE,"MinorAB");
		} else std::sprintf(buffer,"%s%s",KURATOWSKI_FILE,"MinorB");
		Output::kuratowskiOutput(BMP,m_ga,buffer,k,info,pathX,pathY,pathW,B.edgeList);
		#endif
				...
				const SListPure<edge>& pathW)
{
	...
	if (info.pxAboveStopX) {
		...
		char buffer[255];
		if (info.minorType & WInfo::A) {
			std::sprintf(buffer,"%s%s",KURATOWSKI_FILE,"MinorAC");
		} else std::sprintf(buffer,"%s%s",KURATOWSKI_FILE,"MinorC");
		Output::kuratowskiOutput(BMP,m_ga,buffer,k,info,pathX,pathY,pathW,C.edgeList);
		#endif
				...
				const SListPure<edge>& pathW)
{
	...
	char buffer[255];
	if (info.minorType & WInfo::A) {
		std::sprintf(buffer,"%s%s",KURATOWSKI_FILE,"MinorAD");
	} else std::sprintf(buffer,"%s%s",KURATOWSKI_FILE,"MinorD");
	Output::kuratowskiOutput(BMP,m_ga,buffer,k,info,pathX,pathY,pathW,D.edgeList);
	#endif
				...
				const node endnodeZ)
{
	...
	char buffer[255];
	if (info.minorType & WInfo::A) {
		std::sprintf(buffer,"%s%s_Z%d",KURATOWSKI_FILE,"MinorAE1",m_dfi[z]);
	} else std::sprintf(buffer,"%s%s_Z%d",KURATOWSKI_FILE,"MinorE1",m_dfi[z]);
	Output::kuratowskiOutput(BMP,m_ga,buffer,k,info,pathZ,pathZ,pathW,E1.edgeList);
	#endif
				...
				const node endnodeZ)
{
	...
	char buffer[255];
	if (info.minorType & WInfo::A) {
		std::sprintf(buffer,"%s%s_Z%d",KURATOWSKI_FILE,"MinorAE2",m_dfi[z]);
	} else std::sprintf(buffer,"%s%s_Z%d",KURATOWSKI_FILE,"MinorE2",m_dfi[z]);
	Output::kuratowskiOutput(BMP,m_ga,buffer,k,info,pathX,pathY,pathZ,E2.edgeList);
	#endif
				...
				const node endnodeZ)
{
	...
	char buffer[255];
	if (info.minorType & WInfo::A) {
		std::sprintf(buffer,"%s%s_Z%d",KURATOWSKI_FILE,"MinorAE3",m_dfi[z]);
	} else std::sprintf(buffer,"%s%s_Z%d",KURATOWSKI_FILE,"MinorE3",m_dfi[z]);
	Output::kuratowskiOutput(BMP,m_ga,buffer,k,info,pathX,pathY,pathW,E3.edgeList);
	#endif
				...
				const node endnodeZ)
{
	...
	if (px != k.stopX && !info.pxAboveStopX) {
		...
		char buffer[255];
		if (info.minorType & WInfo::A) {
			std::sprintf(buffer,"%s%s_Z%d",KURATOWSKI_FILE,"MinorAE4",m_dfi[z]);
		} else std::sprintf(buffer,"%s%s_Z%d",KURATOWSKI_FILE,"MinorE4",m_dfi[z]);
		Output::kuratowskiOutput(BMP,m_ga,buffer,k,info,pathX,pathY,pathW,E4.edgeList);
		#endif
				...
				const node endnodeZ)
{
	...
	#ifdef FILE_OUTPUT
	char buffer[255];
	std::sprintf(buffer,"%s%s_Z%d",KURATOWSKI_FILE,"MinorE5",m_dfi[z]);
	Output::kuratowskiOutput(BMP,m_ga,buffer,k,info,pathX,pathY,pathW,E5.edgeList);
	#endif
		...
		if (z == info.w) {
			...
			for (itZEndnode = externE.endnodes.begin(); itZEndnode.valid();
				...
				if (!externalPath.empty()) {
					...
					pathZ.clear();
					#ifdef STATISTICS
					stat << "\nMinor E: adjToLowestNodeBelow[" << m_dfi[z] << "] = " << m_dfi[temp->theNode()];
					#endif
				...
				SList<KuratowskiWrapper>& output)
{
	...
	for (itAll=allKuratowskis.begin(); itAll.valid(); ++itAll) {
		...
		static int number = 0;
		stat << "\n------------------------------------------------------------";
		stat << "\n\textract " << (++number) << ". kuratowski structure (on node " << k.V_DFI << ")";
		stat << "\n------------------------------------------------------------";
		#endif
		...
		for (itXEndnode = k.stopXEndnodes.begin(); itXEndnode.valid(); ++itXEndnode) {
			...
			#ifdef STATISTICS
			stat << "\nadjToLowestNodeBelow[" << m_dfi[k.stopX] << "] = " << m_dfi[temp->theNode()];
			#endif
			...
			for (itYEndnode = k.stopYEndnodes.begin(); itYEndnode.valid(); ++itYEndnode) {
				...
				#ifdef STATISTICS
				stat << "\nadjToLowestNodeBelow[" << m_dfi[k.stopY] << "] = " << m_dfi[temp->theNode()];
				#endif
...
#ifdef STATISTICS
// statistical output
void ExtractKuratowskis::statisticsOutput(
				const SListPure<KuratowskiStructure>& allKuratowskis)
{
	SListConstIterator<KuratowskiStructure> itAll;
	SListConstIterator<WInfo> itInfo;
	
	// consider all different kuratowski structures on a node
	for (itAll=allKuratowskis.begin(); itAll.valid(); ++itAll) {
		const KuratowskiStructure& k(*itAll);
		
		stat << "\n-----------STATISTICS in ExtractKuratowskis------------------";
		stat << "\nstopX=" << m_dfi[k.V];
		stat << "\nRReal=" << m_dfi[k.RReal]; // real root
		stat << "\nstopX=" << m_dfi[k.stopX];
		stat << "\nstopY=" << m_dfi[k.stopY];
		
		stat << "\n\nhighest facepath: " << flush;
		ListConstIterator<adjEntry> itt;
		for (itt=k.highestFacePath.begin(); itt.valid(); ++itt)
			stat << m_dfi[(*itt)->theNode()] << ",";
		
		stat << "\nexternal face path: ";
		SListConstIterator<adjEntry> itAdj;
		for (itAdj=k.externalFacePath.begin();itAdj.valid();++itAdj)
			stat << m_dfi[(*itAdj)->theNode()] << ",";
		stat << "\nhighestXYPaths: ";
		SListConstIterator<SListPure<adjEntry> > ite;
		...
		stat << "\n\nexternE: ";
		SListConstIterator<ExternE> it;
		for (it=k.externE.begin(); it.valid(); ++it)
			stat << m_dfi[(*it).theNode] << ",";
