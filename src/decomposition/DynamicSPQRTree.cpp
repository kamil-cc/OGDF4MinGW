...
DynamicSkeleton::DynamicSkeleton (const DynamicSPQRTree* T, node vB) : m_owner(T), Skeleton(vB)
{
...
List<node> DynamicSPQRTree::nodesOfType (NodeType t) const
{
	TNodeType tt;
	switch (t) {
		case SNode: tt = SComp; break;
		case PNode: tt = PComp; break;
		case RNode: tt = RComp; break;
	}
	List<node> L;
	node vT;
	forall_nodes (vT,m_T) {
