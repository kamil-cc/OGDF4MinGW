...
class TreeLayout::AdjComparer : public Comparer<adjEntry>
{
public:
	...
	int compare(const adjEntry &adjX, const adjEntry &adjY) {
...
void TreeLayout::setRoot(GraphAttributes &AG, Graph &tree)
{
	...
	forall_nodes(v,tree)
	{
		...
		if(root == 0) {
			undoReverseEdges(AG);
			THROW_PARAM(PreconditionViolatedException, pvcForest);
		}
...
{
	OGDF_ASSERT(&tree == &((const Graph&)AG));
