 ...
 * last checkin:
 *   $Author: klein $
 *   $Date: 2007-11-14 16:15:25 +0100 (Mi, 14 Nov 2007) $
 ***************************************************************/
...
labelStruct::labelStruct(node parent, node cutvertex, paStopCause sc){
	m_parent = parent;
	m_head = cutvertex;
	m_stopCause = sc;
};

bool labelStruct::isBLabel(){
	if (m_parent != 0)
		return true;
	return false;
};

bool labelStruct::isCLabel(){
	if (m_parent == 0)
		return true;
	return false;
};

node labelStruct::getPendant(int nr){
	if (nr < m_pendants.size())
		return (*(m_pendants.get(nr)));
	return 0;
};

node labelStruct::getFirstPendant(){
	if (m_pendants.size() > 0)
		return m_pendants.front();
	return 0;
};

node labelStruct::getLastPendant(){
	if (m_pendants.size() > 0)
		return m_pendants.back();
	return 0;
};

int labelStruct::size(){
...
void labelStruct::removePendant(ListIterator<node> it){
	m_pendants.del(it);
};

void labelStruct::removeFirstPendant(){
	if (m_pendants.size() > 0){
		m_pendants.popFront();
	}
};

void labelStruct::addPendant(node pendant){
	m_pendants.pushBack(pendant);
};

void labelStruct::deleteAllPendants(){
	m_pendants.clear();
};

node labelStruct::parent(){
	if (m_parent != 0)
		return m_parent;
	return m_head;
};

node labelStruct::head(){
	return m_head;
};
...
{
	DynamicBCTree* bcTreeTemp = new DynamicBCTree(*m_pGraph, true);
	...
	List<node>* getConnected = new List<node>();
...
void PlanarAugmentation::augment()
{
	...
	if (rootPendant != 0){
		...
		node bAdjNode = rootPendant->firstAdj()->twin()->theNode();
...
{	
	...
	if (stopCause == paPlanarity){
		...
		if (m_pBCTree->DynamicBCTree::parent(newPendant) == 0){
			...
			node newRoot = (*(m_adjNonChildren[newPendant].begin()))->twin()->theNode();
		...
		if (m_pBCTree->typeOfBNode(bcNode) == BCTree::CComp){
			...
			if (m_pBCTree->numberOfNodes(bcNode) > 4){
				...
				while (!found && childIt.valid()){
					if (m_pBCTree->find((*childIt)->twin()->theNode()) != last){
						found = true;
						adjBCNode = m_pBCTree->find((*childIt)->twin()->theNode());
					}
					childIt++;
				}
...
bool PlanarAugmentation::planarityCheck(node v1, node v2)
{
	...
	if (v1->firstAdj()->twin()->theNode() == v2){
		return true;
	}
...
{	
	...
	if (cutvertex == 0){
		...
		while (m_pBCTree->DynamicBCTree::bcproper(adj->twin()->theNode()) != v)
			adj = adj->cyclicSucc();
		
		nodeAdjToCutVertex = adj->twin()->theNode();
		...
		if (m_pBCTree->bComponent(nodeAdjToCutVertex, adj->twin()->theNode()) == v){
			found = true;
			nodeAdjToCutVertex = adj->twin()->theNode();
		}
		else{
...
void PlanarAugmentation::joinPendants(label& l)
{
	...
	SList<edge>* newEdges = new SList<edge>();
...
void PlanarAugmentation::connectInsideLabel(label& l)
{
	...
	if (ancestor == 0){
		...
		while ((!found) && (adjIt.valid())){
			...
			if (m_pBCTree->find((*adjIt)->twin()->theNode()) != wrongAncestor){
				ancestor = m_pBCTree->find((*adjIt)->twin()->theNode());
				found = true;
			}
			adjIt++;
		}
	...
	v2 = adjToCutvertex(ancestor, head);
...
{
	label l = new labelStruct(0, cutvertex, whyStop);
	l->addPendant(p);
	m_belongsTo[p] = l;
...
void PlanarAugmentation::updateAdjNonChildren(node newBlock, SList<node>& path)
{
	...
	while (childIt.valid()){
		if (m_pBCTree->find((*childIt)->twin()->theNode()) == newBlock){
	...
	while (pathIt.valid()){
		...
		if (*pathIt != newBlock){
			if (*pathIt == m_pBCTree->find(*pathIt)){
				...
				while (childIt.valid()){
					if (m_pBCTree->find((*childIt)->twin()->theNode()) == (*pathIt)){
...
void PlanarAugmentation::updateNewEdges(SList<edge>* newEdges)
{
	...
	while (edgeIt.valid()){
		...
		&& (m_pBCTree->m_bNode_degree[newBlock] == 1)) {
			...
			node newRoot = 0;
			newRoot = (*(m_adjNonChildren[newBlock].begin()))->twin()->theNode();
