 ...
 * last checkin:
 *   $Author: klein $ 
 *   $Date: 2007-11-15 10:46:52 +0100 (Do, 15 Nov 2007) $ 
 ***************************************************************/
...
//for the input
//void ClusterPlanarizationLayout::call(Graph& G, UMLGraph& umlGraph, ClusterGraph& cGraph)
	...
	if (!cplanar) 
	{
				...
				(CCPE.errCode() == CconnectClusterPlanarEmbed::nonCPlanar) )
		{
			...
			while (itE.valid())
			{
				...
				//hier muss man noch die Typen sichern
				GW.delEdge(e);
			...
			bool subPlanar = CCP.embed(*workCG, GW);
		...
		{
			if (!connect)
			THROW_PARAM(PreconditionViolatedException, pvcClusterPlanar);
		}
