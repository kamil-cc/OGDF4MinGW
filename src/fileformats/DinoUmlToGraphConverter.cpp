 ...
 * last checkin:
 *   $Author: klein $ 
 *   $Date: 2007-11-14 16:16:46 +0100 (Mi, 14 Nov 2007) $ 
 ***************************************************************/
...
namespace ogdf {
	...
	DinoUmlToGraphConverter::DinoUmlToGraphConverter(const char *fileName)
	{
...
#ifdef OGDF_DEBUG
		// Create log file
		m_logFile = new ofstream("umlToGraphConversionLog.txt", ios::out);
#endif
		...
		if (!createDiagramGraphs(m_diagramGraphs)){
...
#ifdef OGDF_DEBUG
		// Write model graph into log file
		*m_logFile << *m_modelGraph;

		// Write diagram graphs into log file
	...
	bool DinoUmlToGraphConverter::createModelGraph(DinoUmlModelGraph &modelGraph){
...
#ifdef OGDF_DEBUG
			*m_logFile << "Error in DinoUmlToGraphConverter::createModelGraph Line: " 
					  << __LINE__
					  << " Message: Root tag is not <XMI>" 
					  << endl;
#endif
		...
		if (!m_xmlParser->traversePath(m_xmlParser->getRootTag(), path, fatherTag)){
			...
			*m_logFile << "Error in DinoUmlToGraphConverter::createModelGraph Line: " 
			  		  << __LINE__
					  << " Message: Path xmiContent, umlModel, umlNamespaceOwnedElement not found!" 
					  << endl;
#endif
																...
																DinoUmlModelGraph &modelGraph)
	{
		...
		while (classifierSon != 0){
...
#ifdef OGDF_DEBUG
				*m_logFile << "Error in DinoUmlToGraphConverter::insertSpecificClassifierNodes Line: " 
				 		  << __LINE__
						  << " Message: Did not find attribute xmi.id of Classifier." 
						  << endl;
#endif
			...
			if (!m_xmlParser->findXmlAttributeObject(*classifierSon, name, nameAttr)){
				...
				*m_logFile << "Error in DinoUmlToGraphConverter::insertSpecificClassifierNodes Line: " 
						  << __LINE__
						  << " Message: Did not find name attribute of Classifier." 
						  << endl;
#endif
		...
		while (associationSon != 0){
...
#ifdef OGDF_DEBUG
				*m_logFile << "Warning in DinoUmlToGraphConverter::traversePackagesAndInsertAssociationEdges Line: " 
						   << __LINE__
						   << " Message: Current association tag does not contain both end tags" 
						   << endl;
#endif
				// Next association
			...
			// one element other than class or interface
				...
				*m_logFile << "Warning in DinoUmlToGraphConverter::traversePackagesAndInsertAssociationEdges Line: " 
						  ...
						  << " Message: Node ";
				if (!node1HE){
					*m_logFile << typeAttr1->m_pAttributeValue->key();
				}
				else{
					*m_logFile << typeAttr2->m_pAttributeValue->key();
				}

				*m_logFile	<< " is not contained in the hash table!"
				 		    << endl;
#endif
			}
			...
			if (m_xmlParser->traversePath(*classSon, path, generalizationTag)){
...
#ifdef OGDF_DEBUG
					*m_logFile << "Warning in DinoUmlToGraphConverter::traversePackagesAndInsertGeneralizationEdges Line: " 
							   << __LINE__
							   << " Message: Current dependency tag does not contain both attributes child and parent." 
							   << endl;
#endif
				...
				// At least one node is not contained in the node hashtable
					...
					*m_logFile << "Warning in DinoUmlToGraphConverter::traversePackagesAndInsertGeneralizationEdges Line: " 
							  ...
							  << " Message: Node ";
					if (!childNodeHE){
						*m_logFile << childAttr->m_pAttributeValue->key();
					}
					else{
						*m_logFile << parentAttr->m_pAttributeValue->key();
					}
		...
		while (currentDependencyTag != 0){
...
#ifdef OGDF_DEBUG
				*m_logFile << "Warning in DinoUmlToGraphConverter::insertDependencyEdges Line: " 
						   << __LINE__
						   << " Message: Current dependency tag does not contain both attributes client and supplier." 
						   << endl;
#endif
				// Next dependency
			...
			// At least one node is not contained in the node hashtable
				...
				*m_logFile << "Warning in DinoUmlToGraphConverter::insertDependencyEdges Line: " 
						  ...
						  << " Message: Node ";
				if (!clientNodeHE){
					*m_logFile << clientAttr->m_pAttributeValue->key();
				}
				else{
					*m_logFile << supplierAttr->m_pAttributeValue->key();
				}
	...
	//
	bool DinoUmlToGraphConverter::createDiagramGraphs(SList<DinoUmlDiagramGraph*> &diagramGraphs)
	{
...
#ifdef OGDF_DEBUG
			*m_logFile << "Error in DinoUmlToGraphConverter::createDiagramGraphs Line: " 
					  << __LINE__
					  << " Message: Root tag is not <XMI>" 
					  << endl;
#endif
		...
		while (currentDiagramTag != 0){
			...
			enum DinoUmlDiagramGraph::UmlDiagramType diagramType;
