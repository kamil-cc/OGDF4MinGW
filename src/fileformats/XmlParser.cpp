								...
								const char *objectBodyName)
{
	...
	while(true) {
		...
		{ // must be the body of an element
			...
			object = new XmlObject(hashString(objectBodyName),pChar); 
		}
		else
		{ // must be a symbol
			if (symbol != xmlKey) {
			...
			switch (symbol) {
			case xmlIntValue:
				object = new XmlObject(key,m_intSymbol);
				break;
			
			case xmlDoubleValue:
				object = new XmlObject(key,m_doubleSymbol);
				break;
			...
			case xmlStringValue: {
				...
				char *pChar = new char[len];
				ogdf::strcpy(pChar,len,m_stringSymbol);
				object = new XmlObject(key,pChar); }
				break;
