...
void GmlParser::createObjectTree(istream &is, bool doCheck)
{
	...
	if (m_rLineBuffer == 0) THROW(InsufficientMemoryException);
	...
	GmlObjectType errorKey)
{
	...
	while(true) {
		...
		case gmlStringValue: {
			...
			if (pChar == 0) THROW(InsufficientMemoryException);
...
bool GmlParser::read(Graph &G)
{
	...
	int minId, maxId;
	GmlObject *graphObject = getNodeIdRange(minId, maxId);
...
bool GmlParser::read(Graph &G, GraphAttributes &AG)
{
	...
	for(; son; son = son->m_pBrother) {
        ...
        case edgePredefKey: {
			...
			edge e = G.newEdge(mapToNode[sourceId],mapToNode[targetId]);
									   ...
									   ClusterGraphAttributes& ACG)
{
	...
	//we have to recount the node id's of read's maptonode
	node v;
	int minId = 0, maxId = 0;
	if (G.numberOfNodes() > 0)
	{
		minId = G.firstNode()->index();
		maxId = G.firstNode()->index();
	}//if not empty
	
	forall_nodes(v, G)
	{
		if (v->index() > maxId) maxId = v->index();
		if (v->index() < minId) minId = v->index();
	}//forall_nodes
	...
	attributedClusterRead(rootObject, CG, ACG, mapToNode);
...
bool GmlParser::readCluster(Graph &G, ClusterGraph& CG)
{
	...
	//we have to recount the node id's of read's maptonode
	node v;
	int minId = G.firstNode()->index(), maxId = G.firstNode()->index();
...
//don't clear, this has to be done by the caller
//	if (CG.numberOfClusters() != 0)
//	{
//		CG.clear();
//	}//if
	...
	clusterRead(rootObject, CG, mapToNode);
			...
			case clusterPredefKey:
				{
					...
					recursiveClusterRead(rootClusterSon, CG, c, mapToNode);
			...
			case clusterPredefKey:
				{
					...
					//recursively read cluster
					recursiveAttributedClusterRead(rootClusterSon, CG, ACG, c, mapToNode);
			...
			case clusterPredefKey:
				{
					...
					recursiveClusterRead(clusterSon, CG, cson, mapToNode);
				}
				break;
			...
			case clusterPredefKey:
				{
					...
					cluster cson = CG.newCluster(c);
					//recursively read child cluster
					recursiveAttributedClusterRead(clusterSon, CG, ACG, cson, mapToNode);
				}
				break;
			case labelPredefKey:
				{
