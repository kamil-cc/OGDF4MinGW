 ...
 * last checkin:
 *   $Author: klein $ 
 *   $Date: 2007-11-13 18:17:58 +0100 (Di, 13 Nov 2007) $ 
 ***************************************************************/
...
class WeightComparer : public Comparer<Tuple2<int,double> >
{
	int compare(const Tuple2<int,double> &x, const Tuple2<int,double> &y)
	{
...
void CircularLayout::assignClustersByBiconnectedComponents(ClusterStructure &C)
{
	...
	while(!Q.empty())
	{
		...
		if(nodeB[componentOf[info.m_vBC]].size() == 2 &&
			...
			info.m_vBC->degree() == 2)
		{
			node wBC = info.m_vBC->firstAdj()->twin()->theNode();
			if(wBC == info.m_predCutBC)
				wBC = info.m_vBC->lastAdj()->twin()->theNode();

			if(wBC->degree() == 2)
			{
				node bBC = wBC->firstAdj()->twin()->theNode();;
				if(bBC == info.m_vBC)
					bBC = wBC->lastAdj()->twin()->theNode();
