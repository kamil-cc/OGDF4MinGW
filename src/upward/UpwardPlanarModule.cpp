	...
	SList<face> &externalFaces)      // returns list of possible external faces
{
	...
	// representative in tree T (the unique tree in F containing no internal
	// vertices of G)
	// if 0, F does not satisfies its required properties
	node noInternalVertex = F.checkForest();

	if (noInternalVertex == 0)
		return false;
	...
	NodeArray<SListPure<adjEntry> > &adjacentEdges)
{
		...
		itBlock.valid(); ++itBlock)
	{
		...
		// exists
		edge eRoot = directSkeletons(T,s,skInfo);
		...
		if(embed)
		{
			...
			OGDF_ASSERT(noInternalVertex != 0);
	...
	bool extFaceIsLeft)
{
	...
	if(T.typeOf(vT) != SPQRTree::RNode) {
		bool testOk = initFaceSinkGraph(M,skInfo[vT]);
		OGDF_ASSERT(testOk);
	}
