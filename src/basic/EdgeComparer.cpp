...
int EdgeComparer::compare(const adjEntry &e1, const adjEntry &e2)
{
	...
	node t1 = (m_PR ?  m_PR->original(e1->twin()->theNode()) : e1->twin()->theNode());
	node t2 = (m_PR ?  m_PR->original(e2->twin()->theNode()) : e2->twin()->theNode());
	...
	if ((!DIsEqual(x1a,x2a)) && (!(DIsEqual(y1a,y2a))))
	{
		OGDF_ASSERT(s1 != s2)
	}
	...
	*/
	...
	if (s1 == s2)
	{
		node uvw = s1->firstAdj()->twin()->theNode();
...
bool EdgeComparer::before(const DPoint u, const DPoint v, const DPoint w)
{
	/*
...
double EdgeComparer::angle(DPoint p, DPoint q, DPoint r) 
{
