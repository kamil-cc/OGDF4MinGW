 ...
 * last checkin:
 *   $Author: klein $ 
 *   $Date: 2007-11-15 14:55:33 +0100 (Do, 15 Nov 2007) $ 
 ***************************************************************/
	...
	forall_nodes(v, *m_pCopy)
	{
		...
		if (v->degree() > 0)
		{
			...
			while (adRun)
			{
				...
				if (adRun->twin()->theNode()->degree() >= minDegree)
					relDegree[v]++;
				adRun = succ;
			}//while
		}//if not isolated
