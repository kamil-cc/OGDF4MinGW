...
void GridLayoutMapped::remap(Layout &drawing)
{
	...
	forall_nodes(v,*m_pPG) {
		...
		//OGDF_ASSERT(m_x[v] % l_gridScale == 0)
		drawing.x(v) = (m_x[v]/l_gridScale) / m_fMapping;
		drawing.y(v) = (m_y[v]/l_gridScale) / m_fMapping;
		//drawing.x(v) = m_x[v] / m_fMapping;
		//drawing.y(v) = m_y[v] / m_fMapping;
	}
}
