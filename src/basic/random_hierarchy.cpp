 ...
 * last checkin:
 *   $Author: gutwenger $ 
 *   $Date: 2007-11-09 13:47:22 +0100 (Fr, 09 Nov 2007) $ 
 ***************************************************************/
...
class CmpTail : public Comparer<bEdge>
{
public:
	int compare(const bEdge &a, const bEdge &b) {
		return (a->tail<b->tail?-1:(a->tail>b->tail?1:cmpId(a,b)));
	}
};


class CmpHead : public Comparer<bEdge>
{
public:
	int compare(const bEdge &a, const bEdge &b) {
	...
	bool planar,bool singleSource,bool longEdges)
{
	...
	List<bEdge> *edgeIn=new List<bEdge>[totNumber];
	List<bEdge> *edgeOut=new List<bEdge>[totNumber];
	if(numberOfLayers) {
		x1=numberOfEdges; x2=0;
		for(n2=fst[1];n2<totNumber;n2++) if(!vrt[n2])
	...
	//forall(actEdge,startEdges) {
		actEdge = *it;
		nextEdge=actEdge;
		while(vrt[nextEdge->head]) nextEdge=nextEdge->next;
