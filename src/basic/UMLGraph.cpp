...
void UMLGraph::adjustHierarchyParents()
{
	...
	forall_nodes(v, *m_pG)
	{
	    ...
	    forall_adj(ae,v)
		{
			if (ae->theNode() != v)
				continue;
			if (m_hierarchyParent[v] == m_hierarchyParent[ae->twin()->theNode()]) //(half)brothers
...
void UMLGraph::computeCliquePosition(node center, double rectMin)
{
	...
	do
	{
		adjNodes.pushBack(ae->twin()->theNode());
		ae = ae->cyclicPred();
	} while (ae != center->firstAdj());
...
void UMLGraph::computeCliquePosition(List<node> &adjNodes, node center, double rectMin)//, const adjEntry &startAdj)
{
	...
	//OGDF_ASSERT(startAdj->theNode() == center)
...
//TODO: Store edge type for all deleted edges
	...
	while (it.valid())
	{
		...
		forall_adj(ad, v)
		{
			if (cliqueNum[ad->twin()->theNode()] == numIt)
			{
...
void UMLGraph::undoStar(node center, bool restoreAllEdges)
{
	...
	//reinsert deleted edges
	
	//all connected edges lead to nodes that have their
	//deleted clique edges targets stored in cliqueEdges
	//TODO: this currently changes the embedding of the
	//original graph, try to correct this
	/*
	adjEntry adj;
    forall_adj(adj, center)
	{
		OGDF_ASSERT(m_replacementEdge[adj->theEdge()])
		node start = adj->twin()->theNode();
		List< CliqueInfo* > &targets = m_cliqueEdges[start];
		//ListIterator<node> itTarget = targets.begin();
		//while (itTarget.valid())
		while (!targets.empty())
		{
			CliqueInfo* cInfo = targets.popFrontRet();
			//we have to be sure that the index is not yet used
			//edge e = m_pG->newEdge(start, cInfo->m_target,cInfo->m_edgeIndex);//(*itTarget));
			delete cInfo;
			//itTarget++;
		}//while
	}//forall_adj
