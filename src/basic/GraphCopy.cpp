 ...
 * last checkin:
 *   $Author: gutwenger $ 
 *   $Date: 2007-11-08 18:57:50 +0100 (Do, 08 Nov 2007) $ 
 ***************************************************************/
		...
		if (m_vCopy[v] != 0)
		{
			...
			forall_adj(adjOr, v)
			{
				if (m_eCopy[adjOr->theEdge()].size() > 0){
					...
					OGDF_ASSERT(cAdj->theNode() == copy(v));
...
edge GraphCopy::newEdge(node v, adjEntry adjEnd, edge eOrig, CombinatorialEmbedding &E)
{
	...
	{
		OGDF_ASSERT(original(v) == eOrig->target())
...
/*
//inserts edge preserving the embedding
//same as above with target v
edge GraphCopy::newEdge(adjEntry adjStart, node v, edge eOrig, CombinatorialEmbedding &E)
{
	OGDF_ASSERT(v != 0 && adjStart != 0)
	OGDF_ASSERT(v->graphOf() == this && adjStart->graphOf() == this);
	OGDF_ASSERT(&(E.getGraph()) == this)

	edge e = E.splitFace(v, adjEnd);
	m_eIterator[e] = m_eCopy[eOrig].pushBack(e);
	m_eOrig[e] = eOrig;
...
/*void GraphCopy::moveToMerger(edge e, edge eMerge)
{
	OGDF_ASSERT(e->graphOf() == this && eMerge->graphOf() == this);

	moveTarget(e,eMerge->source());

	// e must be the first edge in the chain of original(e), otherwise we
	// might not prepend eMerge to this chain!
	OGDF_ASSERT(m_eIterator[e] == m_eCopy[m_eOrig[e]].rbegin());

	m_eIterator[eMerge] = m_eCopy[m_eOrig[e]].pushBack(eMerge);
}*/
...
void GraphCopy::delCopy(node v)
{
	...
	adjEntry adj;
	forall_adj(adj,v)
		m_eCopy[m_eOrig[adj->theEdge()]].clear();
