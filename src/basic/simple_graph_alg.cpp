...
bool isConnected(const Graph &G)
{
	...
	dfsIsConnected(G.firstNode(),visited,count);
	return (count == G.numberOfNodes());
}


static void dfsMakeConnected(node v,
	...
	node &vMinDeg)
{
	...
	forall_adj_edges(e,v) {
		node w = e->opposite(v);
		if (!visited[w]) {
			if (w->degree() < vMinDeg->degree())
				vMinDeg = w;
			dfsMakeConnected(w,visited,vMinDeg);
		}
	}
}
...
void makeConnected(Graph &G, List<edge> &added)
{
	...
	forall_nodes(v,G) {
		if (!visited[v]) {
			vMinDeg = v;
			dfsMakeConnected(v,visited,vMinDeg);
			if (pred) added.pushBack(G.newEdge(pred,vMinDeg));
			pred = vMinDeg;
		}
	}
}
