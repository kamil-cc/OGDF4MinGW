 ...
 * last checkin:
 *   $Author: gutwenger $ 
 *   $Date: 2007-11-12 22:22:15 +0100 (Mo, 12 Nov 2007) $ 
 ***************************************************************/
...
bool MixedModelBase::hasLeft (int k) const
{
	...
	return (it.valid() && (*it).m_adj->twin()->theNode() == m_mmo.m_left[k]);
}
...
bool MixedModelBase::hasRight(int k) const
{
	...
	return (it.valid() && (*it).m_adj->twin()->theNode() == m_mmo.m_right[k]);
}
	...
	ShellingOrderModule &compOrder)
{
	...
	OGDF_ASSERT(PlanarModule().planarityTest(m_PG));
	...
	for (int k = 1; k <= m_mmo.length(); ++k)
	{
		...
		for (int i = 1; i <= V.len(); ++i)
		{
			...
			do {
				...
				adj = adj->cyclicSucc();
			} while (m_iops.marked(adj) || (m_mmo.rank(adj->twin()->theNode()) <= k && adj != adjR));

			for ( ; m_iops.marked(adj) || m_mmo.rank(adj->twin()->theNode()) > k; adj = adj->cyclicSucc())
				if (exists(adj))
			...
			bool has_er = (adjR != 0);
			if (adjL && adjL == adjR) {
				has_el = (adjL->twin()->theNode() != cr);
				has_er = !has_el;
			}

			// determine left(k) and right(k)
			if (i == 1)       m_mmo.m_left [k] = (has_el) ? adjL->twin()->theNode() : cl;
			if (i == V.len()) m_mmo.m_right[k] = (has_er) ? adjR->twin()->theNode() : cr;
...
void MixedModelBase::removeDeg1Nodes()
{
	...
	forall_nodes(v,m_PG) {
		...
		if ((mark[v] = (v->degree() == 1)) == true) {
			node w = v->firstAdj()->twin()->theNode();
	...
	for (int k = 1; k <= m_mmo.length(); ++k)
	{
		...
		for (int i = 1; i <= V.len(); ++i)
		{
			...
			bool onlyLeft = (m_iops.in(v) == 2 && 
				i >= 2 && m_iops.inpoints(v).front().m_adj->twin()->theNode() == V[i-1] &&
					m_iops.marked(m_iops.inpoints(v).back().m_adj));
			bool onlyRight = (m_iops.in(v) == 2 && 
				i < V.len() && m_iops.inpoints(v).back().m_adj->twin()->theNode() == V[i+1] &&
			...
			if (m_iops.out(v) >= 1)
			{
				...
				if (m_iops.in(v) == 2) {
					...
					if (m_iops.inpoints(v).front().m_adj->twin()->theNode() != vl) {
						outL = outPlus;
						deltaR = 1;
...
void MixedModelBase::computeXCoords()
{
	...
	forall_nodes(v,m_PG) {
		...
		for(it = m_iops.outpoints(v).begin(); it.valid(); ++it) {
			if (!m_iops.marked((*it).m_adj))
				high[v] = max(m_mmo.rank((*it).m_adj->twin()->theNode()), high[v]);
		}
	}
	...
	for(k = 2; k <= m_mmo.length(); ++k)
	{
		...
		if ((it = m_nextRight[cl]).valid())  {
			dxl = (*it).m_dx;
			if ((*it).m_adj->twin()->theNode() != z1)
				dxl++;
			else
				m_nextRight[cl] = m_iops.prevRealOut(cl,it);
		...
		if ((it = m_nextLeft[cr]).valid()) {
			dxr = (*it).m_dx;
			if ((*it).m_adj->twin()->theNode() != Vk[p])
				dxr--;
			else
				m_nextLeft[cr] = m_iops.nextRealOut(cr,it);
		...
		if (!m_iops.isChain(z1))
		{
			...
			node ct = ip_ct.m_adj->twin()->theNode();
			...
			for (v = prev[ct]; v != cl; v = prev[v]) {
				x[v] -= sum;
				if (m_nextRight[v].valid() && (*m_nextRight[v]).m_adj->twin()->theNode() == z1) {
...
/*	const InOutPoint &inpoint(int j, list_item it) {
		return iops.inpoints(z(j),it);
	}*/
	...
	if (m_itIpNext.valid() && m_iops.marked((*m_itIpNext).m_adj)) {
		...
		if (m_lookAheadX <= ipX) {
			...
			if(ipLookAhead.m_dx < 0) {
				m_lookAheadNextX = m_x[ipLookAhead.m_adj->twin()->theNode()] + outpoint(ipLookAhead).m_dx;
			} else {
		...
		if (m_onBase) {
			...
			if (!m_itIp.valid()) {
				...
				m_xNext = (marked(ip.m_adj)) ? (m_x[z(m_i)] + ip.m_dx) :
					(m_x[ip.m_adj->twin()->theNode()] + outpoint(ip).m_dx);
			}
			...
			if (m_itIpNext.valid() && ip.m_dx < 0) {
				...
				m_xNext = (marked(m_ipNext.m_adj)) ? (m_x[z(m_i)] + m_ipNext.m_dx) :
					(m_x[m_ipNext.m_adj->twin()->theNode()] + outpoint(m_ipNext).m_dx);
			} else {
				m_xNext = (marked(ip.m_adj)) ? (m_x[z(m_i)] + ip.m_dx + 1) :
					(m_x[ip.m_adj->twin()->theNode()] + outpoint(ip).m_dx + 1);
			}
		...
		for(node v = cl; v != next[cr]; v = next[v])
		{
			...
			if (v == cl) {
				if (!(itFirst = m_leftOp[k]).valid())
					itFirst = out.begin();
				else if ((*itFirst).m_adj->twin()->theNode() != Vk[1])
					++itFirst;

				for(itLast = itFirst; itLast.valid() && (m_iops.marked((*itLast).m_adj) ||
					(*itLast).m_adj->twin()->theNode() == Vk[1]); ++itLast) ;

			} else if (v == cr) {
				itLast = m_rightOp[k];
				if (itLast.valid() && (*itLast).m_adj->twin()->theNode() == Vk[p])
					++itLast;
				itFirst = (itLast.valid()) ? itLast.pred() : out.rbegin();

				while(itFirst.valid() && (m_iops.marked((*itFirst).m_adj) ||
					(*itFirst).m_adj->twin()->theNode() == Vk[p]))
					--itFirst;
			...
			for (ListConstIterator<InOutPoint> it = itFirst; it != itLast; ++it) {
				...
				if (m_iops.marked(op.m_adj))
					...
					setY.checkYCoord(x[v]+op.m_dx, y[v]+op.m_dy,
						(op.m_dx == 0 && op.m_dy == 0 && x[v] == x[op.m_adj->twin()->theNode()]));
			}
		}
...
void MixedModelBase::postprocessing1()
{
	...
	for(int k = 2; k <= m_mmo.length(); ++k) {
		...
		if (!m_iops.marked(adjL) && !m_iops.marked(adjR) &&
			x[adjL->twin()->theNode()] + m_iops.pointOf(adjL->twin())->m_dx < x[v] &&
			x[adjR->twin()->theNode()] + m_iops.pointOf(adjR->twin())->m_dx == x[v]+1 &&
			m_gridLayout.y(adjR->twin()->theNode()) < m_gridLayout.y(v))
		{
			x[v] += 1;
...
void MixedModelBase::print(ostream &os, const InOutPoint &iop)
{
	if(iop.m_adj)
		os << "[(" << m_PG.original(iop.m_adj->theNode()) << "," <<
			m_PG.original(iop.m_adj->twin()->theNode()) << ")," <<
			iop.m_dx << "," << iop.m_dy << "]";
	else
		os << "[ ]";
}
