	...
	double baseRatio)                      // length of the base = baseRatio*size(extface)
{
	...
	m_vRight     = m_adjRight->twin()->theNode();
	...
	{
		v = adj->twin()->theNode(); w = adj->theNode();
		m_onOuter[w] = true;
		edgeToContour(adj);
...
{
	node v = adj->twin()->theNode(), w = adj->theNode();
...
void ComputeBicOrder::getAdjNodes(node v, SListPure<node> &L)
{
	...
	L.clear();
	L.pushBack((v != m_vLeft) ? prev(v) : m_adjLeft->twin()->theNode());

	if (m_deg[v] >= 3) {
		adjEntry adj;
		for (adj = adjEnd; adj != adjStart; adj = adj->cyclicPred())
			L.pushBack(adj->twin()->theNode());
		L.pushBack(adjStart->twin()->theNode());
	}
...
void ComputeBicOrder::removeNextNode(ShellingOrderSet &V)
{
	...
	if (m_virtEdge[prev(m_nextV)] == true) {
		V.left(m_prevPred[m_nextV]->twin()->theNode());
	...
	if (m_virtEdge[m_nextV] == true) {
		V.right(m_nextSucc[m_nextV]->twin()->theNode());
