 ...
 * last checkin:
 *   $Author: klein $ 
 *   $Date: 2007-10-30 11:01:27 +0100 (Di, 30 Okt 2007) $ 
 ***************************************************************/
...
public:
	IOPoints() { }
	IOPoints(const Graph &G) : m_in(G), m_out(G), m_depth(G,0), m_height(G,0),
		m_pointOf(G,0), m_mark(G,false) { }
	...
	bool marked(const PlanRep &PG, node v) {
	...
	ListConstIterator<InOutPoint> firstRealOut(node v) const {
		return searchRealForward(v,m_out[v].begin());
	}

	ListConstIterator<InOutPoint> lastRealOut(node v) const {
		return searchRealBackward(v,m_out[v].rbegin());
	}

	ListConstIterator<InOutPoint> nextRealOut(node v, ListConstIterator<InOutPoint> it) const {
		return searchRealForward(v,it.succ());
	}

	ListConstIterator<InOutPoint> prevRealOut(node v, ListConstIterator<InOutPoint> it) const {
		return searchRealBackward(v,it.pred());
	}
...
private:
	...
	ListConstIterator<InOutPoint> searchRealForward (node v, ListConstIterator<InOutPoint> it) const;
	ListConstIterator<InOutPoint> searchRealBackward(node v, ListConstIterator<InOutPoint> it) const;
};
