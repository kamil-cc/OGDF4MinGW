...
{	
	...
	OGDF_ASSERT(isConnected(G) 		== true);
	...
	node v2 = firstAdj->cyclicPred()->twin()->theNode();	
	...
	while (cto.isPossible()){
		...
		if (cto.isNode()){
			...
			V[1] = vk;
			cl = (adjPred[vk])->twin()->theNode();
			cr = (adjSucc[vk])->twin()->theNode();
