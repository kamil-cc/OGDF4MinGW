	...
	ClusterGraphCopyAttributes &AGC)
{
	...
	/*os << "Variables:\n";
	os << "d_e  : 0,...," << nEdges-1 << "\n";
	os << "x_v  : " << m_vertexOffset << ",...," << m_vertexOffset+nRealVertices-1 << "\n";
	os << "x_s  : " << m_segmentOffset << ",...," << m_segmentOffset+nSegments-1 << "\n";
	os << "b_v  : " << balancedOffset << ",...," << balancedOffset+nBalanced-1 << "\n";
	os << "l_c  : " << m_clusterLeftOffset << ",...," << m_clusterLeftOffset+nClusters-1 << "\n";
	os << "r_c  : " << m_clusterRightOffset << ",...," << m_clusterRightOffset+nClusters-1 << "\n";*/
	...
	forall_edges(e,H)
	{
		...
		if(dCol >= 0) {
			...
			// d_(u,v) - x_u + x_v >= 0
			//os << currentRow << ":  d_" << e << " - x_" << u << " + x_" << v << " >= 0\n";
	...
	//os << "\nSpacing-Constraints:\n";
	// only for debugging:
	//Array2D<int> clLocate(0,nClusters-1,0,nSegments-1,0);
	//Array2D<int> crLocate(0,nClusters-1,0,nSegments-1,0);
	...
	for(i = 0; i < k; ++i)
	{
		...
		// only for debugging:
		/*ListConstIterator<Tuple2<int,double> > itT;
		for(itT = L.begin(); itT.valid(); ++itT) {
			int col = (*itT).x1();

			if(col >= m_clusterLeftOffset && col < m_clusterLeftOffset+nClusters) // cl
			{
				int ci = col - m_clusterLeftOffset;

				ListConstIterator<Tuple2<int,double> > itT2;
				for(itT2 = itT.pred(); itT2.valid(); --itT2) {
					int col2 = (*itT2).x1();
					if(col2 >= m_segmentOffset && col2 < m_segmentOffset+nSegments) // segment
					{
						int si = col2 - m_segmentOffset;
						int &loc = clLocate(ci,si);

						OGDF_ASSERT(loc == 0 || loc == -1);
						loc = -1;
					}
				}

				for(itT2 = itT.succ(); itT2.valid(); ++itT2) {
					int col2 = (*itT2).x1();
					if(col2 >= m_segmentOffset && col2 < m_segmentOffset+nSegments) // segment
					{
						int si = col2 - m_segmentOffset;
			...
			if(col >= m_clusterRightOffset && col < m_clusterRightOffset+nClusters) // cr
			{
				int ci = col - m_clusterRightOffset;
			...
			it2.valid(); it1 = it2, ++it2)
		{
			...
			//os << currentRow << ":  x[" << vCol << "] - x[" << uCol << "] >= " << rightHandSide[currentRow] << "\n";
	...
	if(nBalanced > 0) {
		...
		forall_nodes(v,H)
		{
			...
			//os << currentRow << ":  b_" << v << " - x_" << v << " + " << f << " * Sum x_u >= 0\n";
