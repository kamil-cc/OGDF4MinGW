...
class AdjacencyComparer : public Comparer<LHTreeNode::Adjacency>
{
	int compare(const LHTreeNode::Adjacency &x, const LHTreeNode::Adjacency &y)
	{
...
: m_copy(CG), m_copyEdge(CG), m_topNode(CG), m_bottomNode(CG),
  m_origNode(*this, 0), m_origEdge(*this, 0),
  m_mark(CG, 0), m_type(*this, ntDummy)
{
	const Graph &G = CG;
...
void ExtendedNestingGraph::buildLayers()
{
	...
	for(i = 0; i < m_numLayers; ++i) {
		...
		for(it = L[i].begin(); it.valid(); ++it) {
			...
			LHTreeNode *vNode =  new LHTreeNode(cNode, *it, type);
			treeNode[*it] = vNode;
...
class LocationRelationshipComparer : public Comparer<RCEdge>
{
public:
	int compare(const RCEdge &x, const RCEdge &y)
	{
	...
	if(neighbourParent != 0) {
		...
		for(int i = 0; i < neighbourParent->numberOfChildren(); ++i) {
			...
			if(vNode != 0) {
				...
				if(src != 0) {
					bool result = tryEdge(src, tgt, G, level);
					OGDF_ASSERT(result);
				}
				src = tgt;
			}
