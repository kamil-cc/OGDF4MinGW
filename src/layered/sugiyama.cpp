...
class WeightComparer {
	const NodeArray<double> *m_pWeight;

public:
	WeightComparer(const NodeArray<double> *pWeight) : m_pWeight(pWeight) { }

	bool less(node v, node w) { return (*m_pWeight)[v] < (*m_pWeight)[w]; }
};
...
void Hierarchy::doInit(const NodeArray<int> &rank)
{
	...
	for(it = edges.begin(); it.valid(); ++it)
	{
		...
		if (rankSrc > rankTgt) {
			m_GC.reverseEdge(e); swap(rankSrc,rankTgt);
		}
		...
		for(int i = 0; i < m_numCC; ++i)
		{
			...
			forall_edges(eCopy,GC)
			{
				...
				if(optimizeHorizEdges) {
					// optimization for edges connecting neighbored nodes
					// on the same layer
					node src = GC.copy(e->source());
					node tgt = GC.copy(e->target());
					if(H.rank(src) == H.rank(tgt) &&
						(H.pos(src) == H.pos(tgt)+1 || H.pos(tgt) == H.pos(src)+1))
						AG.bends(e).clear();
				}
