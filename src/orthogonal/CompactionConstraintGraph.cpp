...
writeGML("c:\\temp\\CCgraphbefore.gml", onExternal);
	...
	OGDF_ASSERT(!sources.empty());
	OGDF_ASSERT(!sinks.empty());
...
writeGML("c:\\temp\\CClastgraph.gml");
	...
	if (!isPlanar) THROW(AlgorithmFailureException);
...
//   embedding, e.g., when computing vidibility arcs and allowing edges
//   with length 0.
