	...
	GridLayoutMapped &drawing)
{
	...
	do {
		...
		Dx.insertVertexSizeArcs(PG, drawing.width(), rc);
		Dx.insertVisibilityArcs(PG, currentSeparation, drawing.x(), drawing.y());
...
#ifdef foutputRC
		String fileName;
		fileName.sprintf("c:\\temp\\ogdl\\Dx_%d.gml",steps);
...
#ifdef foutputRC
		fileName.sprintf("c:\\temp\\ogdl\\Gx_%d.gml",steps);
		...
		Dy.insertVertexSizeArcs(PG, drawing.height(), rc);
		Dy.insertVisibilityArcs(PG, currentSeparation, drawing.y(), drawing.x());
...
#ifdef foutputRC
		fileName.sprintf("c:\\temp\\ogdl\\Dy_%d.gml",steps);
...
#ifdef foutputRC
		fileName.sprintf("c:\\temp\\ogdl\\Gy_%d.gml",steps);
		PGUml.writeGML(fileName,OR,drawing);
#endif
...
#ifdef foutputMD
		fileName.sprintf("c:\\temp\\ogdl\\Gx_%d.txt",steps);
...
#ifdef foutputMD
		fileName.sprintf("c:\\temp\\ogdl\\Gy_%d.txt",steps);
		writeGridDrawing(fileName, PG, drawing);
	...
	)
{
...
#ifdef OGDF_DEBUG
		m_flow[eDual] = currentLength;
#endif
		/*OGDF_ASSERT(lowerBound[eDual] > 0 || D.typeOf(e) == cetReducibleArc
			|| D.typeOf(e) == cetFixToZeroArc || D.typeOf(e) == cetMedianArc);*/
	}//forall Gd edges
	...
	// make sure that we have already a feasible solution when applying
	// improvement heurisitics
	/*OGDF_ASSERT_IF(dlConsistencyChecks,
		!improvementHeuristics || MinCostFlowModule::checkComputedFlow(
			dual,lowerBound,upperBound,cost,supply,m_flow));*/
	OGDF_ASSERT(
		!improvementHeuristics || MinCostFlowModule::checkComputedFlow(
			dual,lowerBound,upperBound,cost,supply,m_flow));
	//
	...
	if (dual.numberOfNodes() == 1)
	{
		...
		bool feasible = mcf.call(dual,lowerBound,upperBound,cost,supply,m_flow);
