 ...
 * last checkin:
 *   $Author: klein $ 
 *   $Date: 2007-11-13 18:25:37 +0100 (Di, 13 Nov 2007) $ 
 ***************************************************************/
...
//#define foutput //write some files
...
EdgeRouter::EdgeRouter(PlanRep& pru, OrthoRep& H, GridLayoutMapped& L, CombinatorialEmbedding& E,
                       RoutingChannel<int>& rou, MinimumEdgeDistances<int>& mid,
                       NodeArray<int>& nodewidth, NodeArray<int>& nodeheight) :
          m_prup(&pru), m_layoutp(&L), m_orp(&H), m_comb(&E),
          m_rc(&rou), m_med(&mid), m_nodewidth(&nodewidth), m_nodeheight(&nodeheight)
{
  init(pru, H, L, rou);
  //m_hasOrSep = false;
  //m_orSep = 0;
...
void EdgeRouter::init(PlanRep& pru, 
				 OrthoRep& H, 
				 GridLayoutMapped& L, 
				 ...
				 bool align)
{
  ...
  //saves glue and connectin point positions
...
void EdgeRouter::call(PlanRep& pru, OrthoRep& H, GridLayoutMapped& L, CombinatorialEmbedding& E,
                      RoutingChannel<int>& rou, MinimumEdgeDistances<int>& mid,
                      NodeArray<int>& nodewidth, NodeArray<int>& nodeheight, bool align)
{
	String msg;
	OGDF_ASSERT(H.check(msg));
	init(pru, H, L, rou, align);
	m_prup = &pru;
	m_layoutp = &L;
	m_orp = &H;
	m_comb = &E;
...
#ifdef foutput
pru.writeGML("routeInput.gml", H, L);
#endif /* foutput */
...
//and in layout previous compaction step guarantees routing channel
  ...
  arighte.init(*m_prup, m_init);
    ...
    //compute the lowe / uppe / lefte / righte for every adjacent edge
    //if generalization exists the node position is already fixed
  {
    //HIER NOCH WEGLASSEN, WENN GENERALISIERUNG !!!!!!!
	node l_v;

//*********************************************************************
//compute LOWER / UPPER / LEFTER / RIGHTER
//*********************************************************************
    ...
    forall_nodes(l_v, pru)
    {
      if ((pru.expandAdj(l_v) != 0) && (pru.typeOf(l_v) != Graph::generalizationMerger) )//check if replaced
      {
	    NodeInfo& inf = infos[l_v];
    	  //hier sollte man abfragen ob gen vorhanden auf vertikalen seiten, dann
	      //ist berechnung nicht notwendig
	      //edges to the left side, pointing towards cage
        const List<edge>& left_in_edges = inf.inList(odNorth); //debug!!!!!!!!!!!!
	    ...
	    const List<edge>& right_in_edges = inf.inList(odSouth); //debug!!!!!!!!!!!!
	    pos_e = 1;
	    for (itE = right_in_edges.begin(); itE.valid(); ++itE)
        {
          ...
          //hier sollte man abfragen ob gen an horizontalen seiten, dann ist bestimung nicht notwendig
        //TOP EDGES
...
//there is an omission of placement here leading to errors
      if (false) //preliminary until umlex4 error checked
        ...
        if ( (pru.expandAdj(v) != 0) && (pru.typeOf(v) != Graph::generalizationMerger) )
        {
          ...
          while (pru.typeOf(fa->twin()->theNode()) == Graph::dummy) fa = fa->faceCycleSucc();
          if ((pru.typeOf(fa->twin()->theNode()) != Graph::highDegreeExpander) &&
              (pru.typeOf(fa->twin()->theNode()) != Graph::lowDegreeExpander) )
              {
                //place(v, m_sep, m_overh);
                continue;
              }//if expander
          //get neighbour node
          node v1 = fa->twin()->theNode();
          //get attachment side
          OrthoDir od = OrthoRep::prevDir(H.direction(fa));
          

          OGDF_ASSERT(pru.expandedNode(v1) != 0);
...
//NORTH SIDE *************************************************
  ...
  if (infos[v].has_gen(odNorth))//gen at left side
  {
    ...
    while (l_it.valid())
	{
      ...
      else {m_abends[outEntry(infos[v], odNorth, pos)] = prob_b1r; infos[v].inc_E_hook(odNorth, odEast);}
        m_agp_y[outEntry(infos[v], odNorth, pos)] = ybase;
  ...
  else 
  {
    ...
    while (l_it.valid()) //&&???!!!
	{
      ...
      {m_abends[outEntry(infos[v], odNorth, pos)] = prob_b1r; infos[v].inc_E_hook(odNorth, odEast);}//else
...
//RIGHT SIDE **************************************************************
  ...
  if (infos[v].has_gen(odSouth))
  {
	...
	if (pos > -1)
	  l_it = infos[v].inList(odSouth).get(pos);//muss unter gen sein, -2???
	else l_it = 0;
	//classify edges
            //***************************************************
	//assign gp value for edges underneath generalization
	...
	while (l_it.valid()) //still some lower edges to bend, ycoord+eps+delta!!!!!!!!!!!!!!!!!!!!!!!!!!!
	{
	  ...
	  if (cp_y(outEntry(infos[v], odSouth, pos)) < infos[v].coord(odWest) - m_sep) //paper E^
		{  m_abends[outEntry(infos[v], odSouth, pos)] = prob_b1r; infos[v].inc_E_hook(odSouth, odWest);}
      else {m_abends[outEntry(infos[v], odSouth, pos)] = prob_b2r; infos[v].inc_E(odSouth, odWest);}
			 ...
			 (infos[v].coord(odEast) - cp_y(outEntry(infos[v], odSouth, pos))) )) 
	{
      ...
      l_it++; pos++;
            ...
            //*******************************************************
	  //assign gp value for bend edges on top of generalization 
      //ander: von oben abziehen!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    while (l_it.valid())
	{
		//check for single/2 bend
      if (cp_y(outEntry(infos[v], odSouth, pos)) > infos[v].coord(odEast) + m_sep) 
		{m_abends[outEntry(infos[v], odSouth, pos)] = prob_b1l; infos[v].inc_E_hook(odSouth, odEast);}
      else {m_abends[outEntry(infos[v], odSouth, pos)] = prob_b2l; infos[v].inc_E(odSouth, odEast);}
  ...
  else
  {
    ...
    while (l_it.valid() && ( infos[v].coord(odEast)  >=
			 (cp_y(outEntry(infos[v], odSouth, pos)) 
                         ...
                         + infos[v].eps(odSouth,odWest)) ))
	{
    if (!check) {infos[v].set_r_lower(cp_y(outEntry(infos[v], odSouth, pos))); check = true;}
	...
	{
      if (cp_y(outEntry(infos[v], odSouth, pos)) <= infos[v].coord(odEast) + m_sep) 
        {m_abends[outEntry(infos[v], odSouth, pos)] = bend_2left; infos[v].inc_E(odSouth, odEast);}//N/O?????!!!!!
      else 
        {m_abends[outEntry(infos[v], odSouth, pos)] = prob_b1l; infos[v].inc_E_hook(odSouth, odEast);}//else
...
//and generalizations for horizontal edges
{
  ...
  if (infos[v].has_gen(odEast))//gen at top side
  {
	...
	if (pos > -1)
	  l_it = infos[v].inList(odEast).get(pos);//muss unter gen sein, -2???
    else { l_it = 0; pos = 0;}
	//classify edges
            //***************************************************
	//assign gp value for edges underneath generalization
		    ...
		    (cp_x(outEntry(infos[v], odEast, pos)) - infos[v].coord(odNorth)) ))
	{
      ...
      infos[v].set_t_right(m_agp_x[outEntry(infos[v], odEast, pos)]); //left - right?????
      if (!check) {check = true; infos[v].set_t_left(m_agp_x[outEntry(infos[v], odEast, pos)]);}
	...
	while (l_it.valid()) //still some lower edges to bend
	{
	  ...
	  if (cp_x(outEntry(infos[v], odEast, pos)) < infos[v].coord(odNorth) - m_sep) //paper E^
		{  m_abends[outEntry(infos[v], odEast, pos)] = prob_b1l; infos[v].inc_E_hook(odEast, odNorth);}
      else { m_abends[outEntry(infos[v], odEast, pos)] = prob_b2l; infos[v].inc_E(odEast, odNorth);}
    ...
    m_abends[outEntry(infos[v], odEast, infos[v].gen_pos(odEast))] = bend_free; infos[v].nbf(odEast)++;
    if (!check) infos[v].set_t_left(m_agp_x[outEntry(infos[v], odEast, infos[v].gen_pos(odEast))]);
    ...
    while (l_it.valid())
	{
      ...
      if (m_acp_x[outEntry(infos[v], odEast, pos)] < infos[v].coord(odSouth) + m_sep) 
        {m_abends[outEntry(infos[v], odEast, pos)] = prob_b2r; infos[v].inc_E(odEast, odSouth);}
      else { m_abends[outEntry(infos[v], odEast, pos)] = prob_b1r; infos[v].inc_E_hook(odEast, odSouth);}
  ...
  else 
  {
			  ...
			  - infos[v].eps(odEast,odNorth)) ))
	{
	  if (cp_x(outEntry(infos[v], odEast, pos)) > infos[v].coord(odNorth) - m_sep)//must be doublebend
        { m_abends[outEntry(infos[v], odEast, pos)] = bend_2left; infos[v].inc_E(odEast, odNorth);}
	  else //may be singlebend
        { m_abends[outEntry(infos[v], odEast, pos)] = prob_b1l; infos[v].inc_E_hook(odEast, odNorth);}
          //ab unterem Rand, oder ab gen, teile Abstand Anzahl Kanten??!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
//to allow possible bend saving by edge flipping at the corner
  ...
  //*********************************************************************
    ...
    if (cp_x(outEntry(infos[v], odEast, pos)) <= infos[v].coord(odSouth) + m_sep) 
      {m_abends[outEntry(infos[v], odEast, pos)] = bend_2right; infos[v].inc_E(odEast, odSouth);}
    else {m_abends[outEntry(infos[v], odEast, pos)] = prob_b1r; infos[v].inc_E_hook(odEast, odSouth);}//else
...
//BOTTOM SIDE **************************************************************
	  ...
	  if (infos[v].has_gen(odWest))
	  {
		...
		if (pos > -1)
		  l_it = infos[v].inList(odWest).get(pos);//muss unter gen sein, -2???
        else {l_it = 0; pos = 0;}
		//classify edges
        //***************************************************
		//assign gp value for edges underneath generalization
              ...
              (cp_x(outEntry(infos[v], odWest, pos)) - infos[v].coord(odNorth)) ))
		{
          ...
          if (firstcheck) {firstcheck = false; infos[v].set_b_left(m_agp_x[outEntry(infos[v], odWest, pos)]);}
		...
		while (l_it.valid()) //still some lower edges to bend, xcoord+eps+delta!!!!!!!!!!!!!!!!!!!!!!!!!!!
		{
		  ...
		  if ( cp_x(outEntry(infos[v], odWest, pos)) < infos[v].coord(odNorth) - m_sep) //paper E^
		    {  m_abends[outEntry(infos[v], odWest, pos)] = prob_b1r; infos[v].inc_E_hook(odWest, odNorth);}
          else {m_abends[outEntry(infos[v], odWest, pos)] = bend_2right; infos[v].inc_E(odWest, odNorth);}
        ...
        infos[v].nbf(odWest)++;
        if (firstcheck) { firstcheck = false; infos[v].set_b_right(m_agp_x[outEntry(infos[v], odWest, infos[v].gen_pos(odWest))]);}
        ...
        while (l_it.valid())
		{
          ...
          else {m_abends[outEntry(infos[v], odWest, pos)] = prob_b2l; 
	  ...
	  else
	  {
			     ...
			     (cp_x(outEntry(infos[v], odWest, pos)) - pos*infos[v].delta(odWest, odNorth) - infos[v].eps(odWest,odNorth)) ))
	    {
	      if (cp_x(outEntry(infos[v], odWest, pos)) > infos[v].coord(odNorth) - m_sep)//must be doublebend
            { m_abends[outEntry(infos[v], odWest, pos)]  = bend_2right;  infos[v].inc_E(odWest, odNorth); }
	      else //may be singlebend
            { m_abends[outEntry(infos[v], odWest, pos)] = prob_b1r; infos[v].inc_E_hook(odWest, odNorth); }
              //ab unterem Rand, oder ab gen, teile Abstand Anzahl Kanten??!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
//South edges	 
    ...
    //hier jetzt falls firstunbend != m_init neu anordnen ab unbend + delta
    //*********************************************************************
...
//TOP SIDE *************************************************
		 ...
		 (cp_x(outEntry(infos[v], odEast, pos)) - pos*infos[v].delta(odEast, odNorth) - infos[v].eps(odEast,odNorth)) ))
  {
    ...
    //CHECK: ab unterem Rand, oder ab gen, teile Abstand Anzahl Kanten??
    m_agp_x[outEntry(infos[v], odEast, pos)] = infos[v].coord(odNorth) 
...
//bottom**********************************************************
    ...
    {m_abends[outEntry(infos[v], odWest, pos)] = prob_b1l; infos[v].inc_E_hook(odWest, odSouth);}//else
...
void EdgeRouter::set_corners(node v)
{
  ...
  adjEntry ae = vinfo->m_corner[0]; e = *ae; w = e->source();//pointing towards north, on left side
  m_prup->setExpandedNode(w, v);
  m_layoutp->x(w) = infos[v].coord(odNorth); m_layoutp->y(w) = infos[v].coord(odWest);
  ae = vinfo->m_corner[1]; e = *ae; w = e->source();
  m_prup->setExpandedNode(w, v);
  m_layoutp->x(w) = infos[v].coord(odNorth); m_layoutp->y(w) = infos[v].coord(odEast);
  ae = vinfo->m_corner[2]; e = *ae; w = e->source();
  m_prup->setExpandedNode(w, v);
  m_layoutp->x(w) = infos[v].coord(odSouth); m_layoutp->y(w) = infos[v].coord(odEast);
  ae = vinfo->m_corner[3]; e = *ae; w = e->source();
  m_prup->setExpandedNode(w, v);
  m_layoutp->x(w) = infos[v].coord(odSouth); m_layoutp->y(w) = infos[v].coord(odWest);
...
//**debug
	/*
ListIterator<int> vdb = edgevalue.begin();
int dbi = 0;
while (vdb.valid()) 
{dbi = *vdb; vdb++;}
ListIterator<edge> edb = l_horz.begin();
int dbd = 0;
while (edb.valid())
{
	dbd = (*edb)->index();
...
//**debugend
	...
	OGDF_ASSERT(bestedge != 0);

	m_newy[v] = min((inf.cage_coord(odEast) - inf.node_ysize() - inf.rc(odEast)), 
                      bestvalue);
				    //(lowe[bestedge] ));
  ...
  else
  {
	...
	{
	//carsten: we place at min(ftop-rctop-horig,cpbest-eps(ebest), why not fbottom+???
                ...
                if (eg)
                    {
                      if (inf.is_in_edge(odWest, inf.gen_pos(odWest))) //position to odEast
                          {
                          //OGDF_ASSERT( !(inf.is_in_edge(odEast, inf.gen_pos(odEast))));
...
//eigentlich muesste man hier schon die Kanten klassifizieren
...
//*************debug
 /* List<edge> vertcopy(l_vert);
  //List<edge> vertlcopy(l_vertl);
  List<int>  vcopyentry;
  bit = l_vertl.begin();
  while (bit.valid())
  {
    vcopyentry.pushBack(l_vert.pos(vert_entry[*bit]));
    bit++;
  }*/
  //*************debug end
  //horizontal position****************************
  if (!l_vert.empty())
  {
    ...
    for (i = 1; i < stop+1; i++)
	{
      ...
      //fehler falls falsch loeschen - check 
...
//test: setze die soehne des genmerge an den oberen Rand
  if (m_mergerSon[v]) 
  {
...
//ende test
//hier sollte im Fall von m_minDelta der korrekte Abstand als Platz / Kanten berechnet werden

  //now we have vertical as well as jorizontal position and can assign both values
  if (horizontal_merger)
...
void EdgeRouter::place(node l_v, int l_sep, int l_overh) 
{
  ...
  // normalise, then adjust the layout information for the cage nodes and bend nodes
    String m;
    String msg;
//OGDF_ASSERT(m_orp->check(m));
  OrthoRep::VertexInfoUML* vinfo = m_orp->cageInfo(l_v);
  //adjEntry ae = vinfo->m_corner[0]; e = *ae; w = e->source();
//m_prup->writeGML(String("placeInput%d.gml",l_v->index()), *m_orp, *m_layoutp);
 ...
 // int gx, gy;
  ...
  //spaeter ersetzen ueber:
//**************************************************************
//  OrthoDir odrun = odNorth; //nextDir..., neighbours 
//NORTH SIDE ***************************************************
  ...
  while (it.valid())
  {
    ...
    OGDF_ASSERT((m_agp_y[ae] != m_init) && (m_agp_x[ae] != m_init));
	    ...
	    switch (abendType(ae))
		{
		  //case prob_b1l:
  	      case bend_1left: //rerouted single bend
               ...
               if ((!acorn) && (ipos == infos[l_v].flips(odNorth, odWest) - 1))
               {
                 ...
                 if (ae2 == (ae2->theEdge()->adjSource())) 
                     {newe2 = addRightBend(ae2->theEdge());vinfo->m_corner[0] = newe2->adjSource();}
                 else {newe2 = addLeftBend(ae2->theEdge());vinfo->m_corner[0] = savedge->adjTarget();}
	      ...
	      case bend_1right: 
               if (!corn)
               {
                 ...
                 else {newe2 = addRightBend(ae2->theEdge());vinfo->m_corner[1] = newe2->adjSource();}
      ...
      {
        //if (m_agp_x[ae] != m_init.0) m_layoutp->x(v) = m_agp_x[ae];
	    //if (m_agp_y[ae] != m_init.0) m_layoutp->y(v) = m_agp_y[ae];
        //OGDF_ASSERT((m_agp_x[ae] != m_init.0) && (m_agp_y[ae] != m_init.0));
      } //else nach oben, aber wir wissen ja, wohin
	ipos++;
	it++;
  }//while
	  ...
	  switch (abendType(ae))
	  {
	    ...
	    case bend_1left:  
             ...
             if ((!acorn) && (ipos == infos[l_v].flips(odEast, odNorth) - 1))
             {
               ...
               edge savedge = ae2->theEdge();
               if (ae2 == (ae2->theEdge()->adjSource())) 
                   {newe2 = addRightBend(ae2->theEdge()); vinfo->m_corner[1] = newe2->adjSource();}
               else {newe2 = addLeftBend(ae2->theEdge()); vinfo->m_corner[1] = savedge->adjTarget();}
               fix_position(newe2->source(), infos[l_v].coord(odNorth), infos[l_v].coord(odEast));
               m_prup->setExpandedNode(newe2->source(), l_v);
             }
		...
		case bend_1right: 
             ...
             if (!corn)
             {
               ...
               else { newe2 = addRightBend(ae2->theEdge()); vinfo->m_corner[2] = newe2->adjSource();}
			 ...
			 if (inedge) newe = addRightBend(e);//auch abhaengig machen von inedge
             else newe = addLeftBend(e);
             newbend = newe->source();
             //source ist immer neuer Knick, aber target haengt von der Richtung ab...
             if (inedge) newglue = newe->target();
          ...
          if ((!corn))
             {
               ...
               edge savedge = ae2->theEdge();
               if (ae2 == (ae2->theEdge()->adjSource())) 
                   {newe2 = addRightBend(ae2->theEdge()); vinfo->m_corner[2] = newe2->adjSource();}
               else {newe2 = addLeftBend(ae2->theEdge()); vinfo->m_corner[2] = savedge->adjTarget();}
               fix_position(newe2->source(), infos[l_v].coord(odSouth), infos[l_v].coord(odEast));
               m_prup->setExpandedNode(newe2->source(), l_v);
             }
               //delete corner after last rerouting
		...
		case bend_1right:
            ...
            if (ipos == 0)
             ...
             if ((!acorn) && (ipos == infos[l_v].flips(odSouth, odWest) - 1))
             {
               ...
               else {newe2 = addRightBend(ae2->theEdge()); vinfo->m_corner[3] = newe2->adjSource();}
...
//WEST SIDE******************************************
  ...
  while (it.valid())
  {
    ...
    OGDF_ASSERT((m_agp_x[ae] != m_init) && (m_agp_y[ae] != m_init));
      ...
      if (abendType(ae) != bend_free)
	  {
	      ...
	      case bend_1left:
            //node right side
             ...
             if (!acorn)
             {
               ...
               edge savedge = ae2->theEdge();
               if (ae2 == (ae2->theEdge()->adjSource())) 
                   { newe2 = addRightBend(ae2->theEdge()); vinfo->m_corner[3] = newe2->adjSource();}
               else {newe2 = addLeftBend(ae2->theEdge()); vinfo->m_corner[3] = savedge->adjTarget();}
               fix_position(newe2->source(), infos[l_v].coord(odSouth), infos[l_v].coord(odWest));
               m_prup->setExpandedNode(newe2->source(), l_v);
             }
               //delete corner after last rerouting
	    ...
	    case prob_b2l:
        case bend_2left: 
                          //adjust glue point to rerouted edges !!!!!!!!!!!!!!!!!!!!!!hier muesste aber +1 stehen
                        fix_position(v, m_agp_x[ae]-leftofs, m_agp_y[ae]);
  ...
  //set new corner before first rerouting
             ...
             if ((!corn) && (ipos == infos[l_v].flips(odWest, odNorth) - 1))
             {
               ...
               else {newe2 = addRightBend(ae2->theEdge());vinfo->m_corner[0] = newe2->adjSource();}
               fix_position(newe2->source(), infos[l_v].coord(odNorth), infos[l_v].coord(odWest));
               m_prup->setExpandedNode(newe2->source(), l_v);
             }
                        if (inedge) newe = addRightBend(e);
...
//hier wird eine Seite immer bevorzugt, wenn beide dasselbe sparen!!!!!!!!!!!!!!!!!!!!!!!!!!
...
//preliminary: reset PlanRep expandedNode values if necessary
    ...
    OGDF_ASSERT(m_prup->typeOf(v) != Graph::generalizationMerger);
	  ...
	  do{
		  ...
		  List<edge>& inedges = infos[v].inList(od); //debug change this again !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      infos[v].get_data(*m_prup, *m_orp, *m_layoutp, v, *m_rc, *m_nodewidth, *m_nodeheight);
  ...
  if (infos[v].has_gen(odNorth)) //obacht: sind alles schon odDir Werte !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  {
...
void EdgeRouter::update_node_info(node v, NodeInfo& inf)
//add after placement
{
	//{num_bend_edges_s, num_bend_free_s, num_routable}
}//update_node_info
...
//and return the number of saved bends
{
  //debug: first, we compute (min(al_21, E12)
	//cout<<"COMPUTE_MOVE: "<<alpha_move(s_to, s_from, v)<<" : "<<infos[v].num_routable(s_from, s_to)<<"\n";
...
int EdgeRouter::beta_move(OrthoDir s_from, OrthoDir s_to, int move_num, node v)
{
 ...
 //could be routed bendfree to an glue point if move_num edges are flipped to s_to
  ...
  int poss_bf = 0; //possible bend free edges
  int ic = 0;
  ...
  switch (s_from)
  {
	...
	case odWest:
		switch (s_to)
		{
			case odSouth: bt1 = prob_b1r;
				          bt2 = prob_b2r;
						  bt3 = bend_1right;
						  bt4 = bend_2right;
						  break;
			case odNorth: bt1 = prob_b1l;
				          bt2 = prob_b2l;
						  bt3 = bend_1left;
						  bt4 = bend_2left;
						  break;
	  ...
	  case odNorth:
		switch (s_to)
		{
			case odWest:  bt1 = prob_b1r;
				          bt2 = prob_b2r;
						  bt3 = bend_1right;
						  bt4 = bend_2right;
						  break;
			case odEast:  bt1 = prob_b1l;
				          bt2 = prob_b2l;
						  bt3 = bend_1left;
						  bt4 = bend_2left;
						  break;
	  ...
	  case odSouth:
		switch (s_to)
		{
			case odEast:  bt1 = prob_b1r;
				          bt2 = prob_b2r;
						  bt3 = bend_1right;
						  bt4 = bend_2right;
						  break;
			case odWest:  bt1 = prob_b1l;
				          bt2 = prob_b2l;
						  bt3 = bend_1left;
						  bt4 = bend_2left;
						  break;
  ...
  {//debug top side to
	...
	//OGDF_ASSERT(ep.valid()); //?not necessary, return 0
	...
	{
      if (down) 
      {ep++; ae = outEntry(infos[v], s_from, ++adjcount);}
      else {ep--; ae = outEntry(infos[v], s_from, --adjcount);}
	  ic++;
    ...
    case odNorth: //two from cases: top or bottom
		 switch (s_from)
         {
		   ...
		   case odWest: 
			    ...
			    break;
		   default: cout<<"unmatched s_to in EdgeRouter::alpha_move\n"; throw AlgorithmFailureException(); break;//error_handler()
         }//switch
		         ...
		         switch (s_from)
				 {
				   ...
				   case odSouth: 
					    if (infos[v].num_bend_free(odWest) != 0)
  					    ...
  					    break;
				   default: cout << "unmatched s_to in EdgeRouter::alpha_move\n"; THROW(AlgorithmFailureException); break;
				 }//switch
		         break;

	default: cout << "unrecognized side in EdgeRouter::alpha_move\n"; THROW(AlgorithmFailureException); break;
  }//switch

  //OGDF_ASSERT(result > -1);
...
/*
should have a function like ..., but  embedding, orthorep....
  ostream& operator<<(ostream& o, const OrthoRep& H)
{
  o<<"OrthogonalRepresentation with "<<Graph(H).numberOfNodes()<<" Nodes \n";
  face f;
  forall_faces(f, H)
  {
    o<<"Face mit Index "<<f->index()<<"\n";
    adjEntry ad = f->first_adj();    
    adEntry adstop = ad;
    do 
    {
      cout<<ad<<": "<<H.angle(ad)*90<<" / "<<H.bend(ad)<<"\n";
      
    } while ((ad = ad->faceCyclSucc())!= adstop);
