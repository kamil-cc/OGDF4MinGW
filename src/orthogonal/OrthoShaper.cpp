				...
				if (adj == adj->theEdge()->adjSource())
				{
					if ( (PG.typeOf(adj->theEdge()) == Graph::generalization) ) 
					{
						   ...
						   )
						{
						  ...
						  //OGDF_ASSERT(PG.typeOf(nextAE->theNode()) == Graph::vertex);
		...
		if (PG.expandAdj(v))
		{
			...
			if (PG.typeOf(v) == Graph::generalizationMerger)
			{
				...
				forall_face_adj(adj,f)
				{
					...
					node w = networkNode[adj->twin()->theNode()];
				...
				node w = networkNode[PG.expandAdj(v)->twin()->theNode()];
                ...
                if (m_multiAlign)
				{
                    ...
                    forall_face_adj(adj, f) //this double iteration slows the algorithm down
                    {
                        if (!PG.faceSplitter(adj->theEdge()))
                        {
                          ...
                          node vt1, vt2;
                          if (PG.expandedNode(srcadj->twin()->theNode())) 
                              vt1 = PG.expandedNode(srcadj->twin()->theNode());
                          else vt1 = srcadj->twin()->theNode();
                          if (PG.expandedNode(tgtadj->twin()->theNode())) 
                              vt2 = PG.expandedNode(tgtadj->twin()->theNode());
                          else vt2 = tgtadj->twin()->theNode();
                          if (vt1 == vt2) 
                          {
					...
					if (allMulti)
					{
				  	    ...
				  	    forall_face_adj(adj, f)
						{
							...
							if (PG.expandedNode(vOpp)) 
							{
								...
								while (PG.expandedNode(adjOut->twin()->theNode()) == vStop)
				...
				forall_face_adj(adj,f)
				{
                    ...
                    //should be: only one bend
                    {
						...
						//depending on the degree (<= 4 => deg 90)
                        {
							...
							//if (nodeType[ networkNode[adj->twin()->theNode()] ] == high)
        ...
        else
        {
            ...
            if (PG.isVertex(v))
            {
				...
				forall_adj_edges(e,w)
				{
					...
					if (m_traditional && (e->source() != w)) THROW(AlgorithmFailureException);
                ...
                if (m_multiAlign && allMulti  && (v->degree()>1))
                {
                    ...
                    forall_adj_edges(e, w)
                    {
						...
						if (PG.expandedNode(vStop)) 
						{
							...
							while (PG.expandedNode(runAdj->twin()->theNode()) == vStop)
								//we are still on vOpps cage
                                  runAdj = runAdj->faceCycleSucc();
						}
						//adjEntry testadj = adjCor[e]->faceCycleSucc()->twin();
...
//#endif
    ...
    {
		cout << "OrthoFormer: couldn't compute reasonable shape \n" << flush;
		bool NoFormerFlow = false;
		OGDF_ASSERT(NoFormerFlow);
		THROW_PARAM(AlgorithmFailureException, afcNoFlow);
    }
