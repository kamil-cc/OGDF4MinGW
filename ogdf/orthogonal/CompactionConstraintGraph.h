 ...
 * last checkin:
 *   $Author: klein $ 
 *   $Date: 2007-11-13 18:03:43 +0100 (Di, 13 Nov 2007) $ 
 ***************************************************************/
...
public:
	...
	void insertVisibilityArcs(
		...
		ATYPE currentSeparation, //currently used separation in flowcompaction
...
private:
	...
	//---------------------------------------------------------
	class SegmentComparer : public Comparer<node>
	{
	public:
	...
	adjEntry adjFirst)
{
	...
	do {
		if ((m_pOR->direction(adj) == m_arcDir ||
			...
			PG.typeOf(adj->twin()->theNode()) == Graph::dummy))
		{
                ...
                if ((sDir.totalAttached() == 1) || (sOppDir.totalAttached() == 1))
                    {
                        ...
                        edge eBungeeOut =  newEdge(vBungee, m_pathNode[cornerDir->twin()->theNode()]);
                        if (m_pathNode[cornerDir->twin()->theNode()] == vMin) exit(1);
                        //bungee, center and outgoing segment may build column if in the middle
...
/*
                        ...
                        edge eFromOut = newEdge(m_pathNode[cornerDir->twin()->theNode()], vMax);
                        m_type[eFromOut] = cetBasicArc; //!!!!!!!!!!!!!!!!!!!!!!!
                        m_cost[eFromOut] = 0; //!!!!!!!!!!!!!!!!!!!
                        m_length[eFromOut] = minDist.epsilon(v,m_arcDir,1);
  */                      
                            }//if sDir case
                       if ( (sOppDir.totalAttached() == 1) && !(m_pathNode[cornerOppDir->twin()->theNode()] == vMin) )
                            {
                        ...
                        edge eBungeeOut =  newEdge(vBungee, m_pathNode[cornerOppDir->twin()->theNode()]);
                        //bungee, center and outgoing segment may build column if in the middle
                        m_type[eBungeeOut] = cetMedianArc; 
                        m_cost[eBungeeOut] = m_bungeeCost; //what about this !!!!!!!!!!!!!!!!!!!
                        m_length[eBungeeOut] = 0;
/*
                        //connect outgoing segment and "right" segment, maybe obsolete
                        edge eFromOut = newEdge(m_pathNode[cornerOppDir->twin()->theNode()], vMax);
                        m_type[eFromOut] = cetBasicArc; //!!!!!!!!!!!!!!!!!!!!!!!
                        m_cost[eFromOut] = 0; //!!!!!!!!!!!!!!!!!!!
	...
	const MinimumEdgeDistances<ATYPE> &minDist)
{
		...
		if(typeLow == Graph::dummy || typeLow == Graph::generalizationExpander) {
			...
			if (nodeLow->degree() == 2) {
				...
				if (adj) {
					...
					if(posDir[adj->theNode()] == posDir[adj2->twin()->theNode()])
					subtractSep = false;
				}
			}
			//if (subtractSep)*/
	...
	forall_nodes(v,PG)
	{
			...
			adj = adj->faceCycleSucc()) 
		{
			...
			ATYPE delta = (isCaseA) ?
				min(abs(posOrthDir[adjPred->theNode()] - posOrthDir[adjPred->twin()->theNode()]), m_sep) :
				min(abs(posOrthDir[adj->theNode()] - posOrthDir[adj->twin()->theNode()]), m_sep);
			ATYPE boundary = (isCaseA) ?
				min(posOrthDir[adjPred->theNode()], posOrthDir[adjPred->twin()->theNode()]) :
				min(posOrthDir[adj->theNode()],     posOrthDir[adj->twin()->theNode()]);
			...
			if (PG.typeOf(adjCross->theEdge()) == Graph::generalization)
			{
				if (isCaseA) {
						...
						m_pOR->angle(adjTwin) == 2)
					{
						...
						node s2 = m_pathNode[adjTwin->cyclicSucc()->twin()->theNode()];
					...
					{
						node s1 = m_pathNode[adjTwin->theNode()];
						node s2 = m_pathNode[adjTwin->cyclicPred()->twin()->theNode()];
						low[s1] = lowReal[s1] - delta; //minDist.delta(v,dirMin,i);
						low[s2] = lowReal[s2] - delta; //minDist.delta(v,dirMin,i);
					}
				}
			...
			adj = adj->faceCycleSucc())
		{
			...
			//ATYPE delta = -posOrthDir[adj->twin()->theNode()] + posOrthDir[adj->theNode()];
			adjEntry adjPred = adj->faceCyclePred();
			ATYPE delta = (isCaseA) ?
				min(abs(posOrthDir[adj->twin()->theNode()] - posOrthDir[adj->theNode()]), m_sep) :
				min(abs(posOrthDir[adjPred->theNode()] - posOrthDir[adjPred->twin()->theNode()]), m_sep);
			ATYPE boundary = (isCaseA) ?
				min(posOrthDir[adj->twin()->theNode()], posOrthDir[adj->theNode()]) :
				min(posOrthDir[adjPred->theNode()],     posOrthDir[adjPred->twin()->theNode()]);
	...
	// compute toplogical numbering of segments as second sorting criteria
	...
	// We use this in order to omitt visibility arcs between segments which
