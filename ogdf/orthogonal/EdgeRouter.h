...
#ifndef OGDF_EDGEROUTER_H
#define OGDF_EDGEROUTER_H
...
enum bend_type {bend_free, bend_1left, bend_1right, bend_2left, bend_2right, //results
                  prob_bf, prob_b1l, prob_b1r, prob_b2l, prob_b2r}; //and preliminary
...
public: 
		...
		EdgeRouter(PlanRep& pru, OrthoRep& H, GridLayoutMapped& L, CombinatorialEmbedding& E,
                   RoutingChannel<int>& rou, MinimumEdgeDistances<int>& med,
                   NodeArray<int>& nodewidth, NodeArray<int>& nodeheight);

		virtual ~EdgeRouter() {};

        void init(PlanRep& pru, OrthoRep& H, GridLayoutMapped& L, RoutingChannel<int>& rou,
			 bool align = false);
		...
		void initialize_node_info(node v, int sep);
		  //! sets values derivable from placement step
		void update_node_info(node v, NodeInfo& inf);
        ...
        adjEntry outEntry(NodeInfo& inf, OrthoDir d, int pos) 
          {if (inf.is_in_edge(d, pos)) return (*inf.inList(d).get(pos))->adjTarget(); 
           else return (*inf.inList(d).get(pos))->adjSource();}//we only bend on outentries
          //! adjEntries for edges in inLists
        adjEntry inEntry(NodeInfo& inf, OrthoDir d, int pos) 
          {if (inf.is_in_edge(d, pos)) return (*inf.inList(d).get(pos))->adjSource(); 
           else return (*inf.inList(d).get(pos))->adjTarget();}
...
private:
        ...
        node oppositeNode(adjEntry ae) {return ae->twin()->theNode();}
