...
ogdf::ListIterator<adjEntry> OGDF_VAR(__LINE__);\
for(OGDF_VAR(__LINE__)=(c)->firstAdj();\
	ogdf::test_forall_adj_entries_of_cluster(OGDF_VAR(__LINE__),(adj));\
OGDF_VAR(__LINE__)=OGDF_VAR(__LINE__).succ())

#define forall_cluster_rev_adj(adj,c)\
ogdf::ListIterator<adjEntry> OGDF_VAR(__LINE__);\
for(OGDF_VAR(__LINE__)=(c)->lastAdj();\
	ogdf::test_forall_adj_entries_of_cluster(OGDF_VAR(__LINE__),(adj));\
OGDF_VAR(__LINE__)=OGDF_VAR(__LINE__).pred())

#define forall_cluster_adj_edges(e,c)\
ogdf::ListIterator<adjEntry> OGDF_VAR(__LINE__);\
for(OGDF_VAR(__LINE__)=(c)->firstAdj();\
	ogdf::test_forall_adj_edges_of_cluster(OGDF_VAR(__LINE__),(e));\
OGDF_VAR(__LINE__)=OGDF_VAR(__LINE__).succ())
...
public:
	...
	//! Creates a new cluster containing the nodes given by \a nodes.
	/**
	 ...
	 * @param parent is the parent of the cluster.
	...
	int treeDepth() const
	{
		...
		if (!m_updateDepth) THROW(AlgorithmFailureException);
		int l_depth = 1;
		cluster c;
	...
	int& clusterDepth(cluster c) const 
	{
		...
		if (m_updateDepth && !m_depthUpToDate)
			computeSubTreeDepth(rootCluster());
		//if depth is requested but not updated
		if (!m_updateDepth) THROW(AlgorithmFailureException);
		//debug check
#ifdef OGDF_DEBUG
		cluster cp = c->parent();
		int lastDepth = c->depth();
		while (cp != 0)
		{
			bool b = (lastDepth == cp->depth()+1);
			OGDF_ASSERT(b)
			lastDepth = cp->depth();
			cp = cp->parent();
		}
		OGDF_ASSERT(lastDepth == 1);
#endif
		//end
