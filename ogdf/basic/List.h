...
// workaround for bug in Visual Studio 5.0
		...
		g_memory.deallocateList(m_head,m_tail,sizeof(ListElement<E>));
	...
	//! Sorts the list using Quicksort and parameterized comparer \a comp.
	template<class C>
...
public:
	...
	/*// removes it wihtout freeing memory
	// By S.Leipert
	void remove(ListIterator<E> it) {
		--m_count;
		ListPure<E>::remove(it);
	}*/
	...
	// sorts list using quicksort and compare element comp
	void quicksort(Comparer<E> &comp) {
		ogdf::quicksortTemplate(*this,comp);
	}
...
template<class E>
void quicksort(List<E> &L, Comparer<E> &comp)
{
	const int n = L.size();
	Array<E> A(n);
...
// sorts list L using quicksort and parameterized compare element comp
template<class E, class C>
void quicksortCT(List<E> &L, C &comp)
{
	const int n = L.size();
	Array<E> A(n);
...
/*template<class E>
void bucketSort(ListPure<E> &L, int l, int h, BucketFunc<E> &f)
{
	if (L.empty()) return;

	Array<ListElement<E> *> bucket(l,h,0);

	ListElement<E> *pX, *pY, *pI;
	for (pX = L.m_tail; pX; pX = pY) {
		pY = pX->m_prev;
		int i = f.getBucket(pX->m_x);
		pX->m_next = pI = bucket[i];
		if (pI) pI->m_prev = pX;
		bucket[i] = pX;
	}
	...
	for (int i = l; i <= h; i++) {
		...
		if (pX) {
			if (pY) {
				pY->m_next = pX; pX->m_prev = pY;
			} else {
				L.m_head = pX; pX->m_prev = 0;
			}
			for(pY = pX; pY->m_next; pY = pY->m_next) ;
		}
	}
