...
class HashElementBase {
	...
	int m_hashValue; //!< The hash value.

public:
	//! Creates a hash element with hash value \a hashValue.
	HashElementBase(int hashValue) : m_hashValue(hashValue) { }
...
public:
	 ...
	 */
	HashElementBase *firstListElement(int hashValue) const {
...
public:
	...
	HashElement(int hashValue, const K &key, const I &info) :
...
template<class K> class DefHashFunc {
	...
	public:	int hash(const K &key) const { return int(key); }
};

//! Specialized default hash function for pointer types.
template<> class DefHashFunc<void *> {
	public:	int hash(const void * &key) const { return (int)(key && 0xffffffff); }
};

//! Specialized default hash function for double.
template<> class DefHashFunc<double> {
	public:	int hash(const double &key) const {
		int dummy;
		return int(INT_MAX*frexp(key,&dummy));
	}
};
