...
#ifndef OREAS_BINARY_HEAP_H
#define OREAS_BINARY_HEAP_H
...
class BinaryHeap : public HeapBase<key, HeapObject>
{
public:
...
template <class key, class HeapObject>
BinaryHeap<key, HeapObject>::BinaryHeap(int startSize)
...
template <class key, class HeapObject>
void BinaryHeap<key, HeapObject>::init(int initSize)
{
...
template <class key, class HeapObject>
void BinaryHeap<key, HeapObject>::clear()
{
	if (m_heapArray) delete[] m_heapArray;
	init(m_startSize);
}
...
template <class key, class HeapObject>
void BinaryHeap<key, HeapObject>::siftUp(int pos)
{
...
template <class key, class HeapObject>
void BinaryHeap<key, HeapObject>::siftDown(int pos)
{
	OGDF_ASSERT( (pos > 0) && (pos <= HeapBase<key,HeapObject>::m_size) );
...
template <class key, class HeapObject>
void BinaryHeap<key, HeapObject>::makeHeap()
{
...
template <class key, class HeapObject>
void BinaryHeap<key, HeapObject>::decreaseKey(int index, key priority)
{
  HeapEntry& he = m_heapArray[index];

  //check if error value
  if (he.m_priority < priority) THROW_PARAM(AlgorithmFailureException, afcIllegalParameter);
...
template <class key, class HeapObject>
HeapObject BinaryHeap<key, HeapObject>::extractMin()
{
...
template <class key, class HeapObject>
void BinaryHeap<key, HeapObject>::insert(HeapObject& ho, key& priority, int* keyUpdate)
{
...
template <class key, class HeapObject>
const BinaryHeap<key, HeapObject>& BinaryHeap<key, HeapObject>::operator=(const BinaryHeap<key, HeapObject>& rhs)
{
	if (this != &rhs)
	{
