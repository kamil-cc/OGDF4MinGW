...
public:
	...
	void sort(const LIST &newOrder) {
		...
		for(; it.valid(); ++it) {
			GraphElement *p = *it;
			if ((p->m_prev = pPred)) pPred->m_next = p;
			pPred = p;
		}
...
public:
	...
	~GraphList() {
		if (m_head) g_memory.deallocateList(m_head,m_tail,sizeof(T));
	}
	...
	void clear() {
		if (m_head) {
			g_memory.deallocateList(m_head,m_tail,sizeof(T));
			m_head = m_tail = 0;
		}
	}
...
ogdf::adjEntry OGDF_VAR(__LINE__);\
for(OGDF_VAR(__LINE__)=(v)->firstAdj();\
	ogdf::test_forall_adj_edges(OGDF_VAR(__LINE__),(e));\
