 ...
 * last checkin:
 *   $Author: klein $ 
 *   $Date: 2007-11-13 17:57:45 +0100 (Di, 13 Nov 2007) $ 
 ***************************************************************/
...
public: 
		...
		NodeInfo(PlanRep& P, OrthoRep& H, GridLayout& L, node v, adjEntry adj, RoutingChannel<int>& rc,
                 NodeArray<int>& nw, NodeArray<int>& nh) : m_adj(adj)
		        { init(); get_data(P, H, L, v, rc, nw, nh); }
		virtual ~NodeInfo() {};
		...
		bool is_in_edge(OrthoDir od, int pos) 
		  ...
		  //set
		void reclassify(OrthoDir s) {};//set m_nbf, nb, m_routable based on bend_type values on s,
		  ...
		  //delta values, due to placement problems, cut to box_size / 2
   		void set_delta(OrthoDir bside, OrthoDir bneighbour, int dval) 
            {switch (bside)
             {
               case odNorth:
               case odSouth: if (dval > box_y_size) {dval = int(floor(((double)box_y_size / 2))) - m_eps[bside][bneighbour];
                                } break;
               case odEast:
               case odWest: if (dval > box_x_size) {dval = int(floor(((double)box_x_size / 2))) - m_eps[bside][bneighbour];
                                } break;
              }//switch
...
//		int compute_rc(box_side b) {cout<<"rc not yet implemented\n";exit(1);}
        ...
        {m_nbe[s_from][s_to] += num;}
		  ...
		  //(needs positions ...)
		void get_data(PlanRep& P, OrthoRep& O, GridLayout& L, node v, RoutingChannel<int>& rc,
