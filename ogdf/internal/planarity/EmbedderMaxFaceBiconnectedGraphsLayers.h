	...
	adjEntry& adjExternal)
{
	...
	node m_rightNode = m_leftNode->firstAdj()->twin()->theNode();
	...
	const node& n /* = 0 */)
{
	                                                   ...
	                                                   : ae->faceCycleSucc()))
	{
		       ...
		       numOfHits = (aeN == m_stop_ae) ? numOfHits + 1 : numOfHits)
		{
			...
			//list of that node.
			if (maxFaceNodes.search(aeN->twin()->theNode()) == -1)
			{
				node orig_aeN_twin_theNode = S.original(aeN->twin()->theNode());
			...
			if (buffer[ae->theEdge()].empty())
			{
				...
				if (S.isVirtual(ae->theEdge()))
				{
					if (!DGcomputed)
					{
						...
						p_dist_f_ref = new NodeArray<T>();
						p_dist_f_ext = new NodeArray<T>();
						p_ae_to_face = new AdjEntryArray<int>(S.getGraph());
				...
				if (frauenlinks)
				{
					...
					//copy buffered embedding reversed into newOrder:
					node m_leftNode = v;
					node m_rightNode = ae->twin()->theNode();
				...
				if (!nodeTreated[ae->twin()->theNode()])
				{
					node orig_ae_twin_theNode = S.original(ae->twin()->theNode());
