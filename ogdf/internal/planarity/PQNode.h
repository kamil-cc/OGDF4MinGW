...
#ifndef OGDF_PQNODE_H
#define OGDF_PQNODE_H
...
public:
	...
	int  childCount(int count) {return m_childCount = count;};
	...
	*/
	int parentType(int newParentType) {return m_parentType = newParentType;};
	...
	int pertChildCount(int count) {return m_pertChildCount = count;};
	...
	// Returns the value of the variable [[m_userDefindeCount]]
	//int  userDefinedCount() const {return m_userDefinedCount;};

	// Sets  [[m_userDefinedCount = count]].
	//int  userDefinedCount(int count) {return m_userDefinedCount = count;};    

	// Increments the [[m_userDefinedCount]] by the value i.
	//void userDefinedIncr(int i) {m_userDefinedCount = m_userDefinedCount + i;}; 

	// Decrements the [[m_userDefinedCount]] by the value d.
	//void userDefinedDecr(int d) {m_userDefinedCount = m_userDefinedCount - d;}; 
	 ...
	 * PQTree is either \a P_NODE, \a Q_NODE or	\a LEAF (see PQNode).
...
protected:
    ...
    /*
	 * Is a number not needed by the
	 * class template PQTree. It is left at the users disposal along
	 * with a few public member functions for manipulating the
	 * \a m_userDefinedCount.
	 */
	//int  m_userDefinedCount;
                               ...
                               PQNode<T,X,Y>* newEnd)
{
	...
	//else
	OGDF_ASSERT(false);//old Endmost not found
	return false;
}
                                ...
                                PQNode<T,X,Y>* newSib)
{
	...
	//else
	OGDF_ASSERT(false);//old Sibling not found
	return false;
}
...
/*
The function [[putSibling]] stores a new sibling at a free sibling pointer
of the node. This is only possible, if the node has at most one sibling.
The function then detects a non used sibling pointer and places [[newSib]]
onto it. [[putSibling]] returns [[0]] if there have been two siblings
detected, occupying the two possible pointers. In this case the new sibling
[[newSib]] cannot be stored. If there was at a maximum one sibling stored,
the function will place [[newSib]] on the free pointer and return either
[[LEFT]] or [[RIGHT]], depending wich pointer has been used.

This function will always scan the pointer to the left brother first.
*/
template<class T,class X,class Y> 
SibDirection PQNode<T,X,Y>::putSibling(PQNode<T,X,Y>* newSib)
{
   if (m_sibLeft == 0)
   {
      m_sibLeft = newSib;
   ...
   //else
	#ifdef OGDF_DEBUG
	if (int(debugLevel) >= int(dlHeavyChecks))
		cout << "ERROR: PQNode::putSibling: Two existing siblings detected"
		 	 << endl
			 << "IdentificationNumber: PQNode = " << m_identificationNumber
			 << "m_sibLeft = " << m_sibLeft->m_identificationNumber
			 << "m_sibRight = " << m_sibRight->m_identificationNumber
			 << endl;
	#endif
	OGDF_ASSERT(false)
	return NODIR;
...
onto it. [[putSibling]] returns [[0]] if there have been two siblings
detected, occupying the two possible pointers. In this case the new sibling
[[newSib]] could not be stored. If there was at a maximum one sibling stored,
the function will place [[newSib]] on the free pointer and return either
[[LEFT]] or [[RIGHT]], depending wich pointer has been used.

This function scans the brother first, which has been specified in the
preference. If the preference has value [[LEFT]], it scans the pointer
to the left brother first. If the value is [[RIGHT]], it scans the pointer
to the right brother first.
*/
template<class T,class X,class Y> 
SibDirection PQNode<T,X,Y>::putSibling(PQNode<T,X,Y>* newSib,int preference)
{
	if (preference == LEFT)
	...
	OGDF_ASSERT(false); //Non existing direction specified.
	return NODIR;
}
