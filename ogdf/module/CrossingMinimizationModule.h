 ...
 *
 */
class CrossingMinimizationModule : public Module, public Timeouter
{
public:
			...
			const EdgeArray<unsigned int> * subgraphs = 0)
	{
		...
		if(!useCost())      cost      = new EdgeArray<int> (PG.original(), 1);
		if(!useForbid())    forbid    = new EdgeArray<bool> (PG.original(), 0);
		if(!useSubgraphs()) subgraphs = new EdgeArray<unsigned int> (PG.original(), 1);
