...
#ifndef OREAS_EDGE_LABEL_H
#define OREAS_EDGE_LABEL_H
...
protected:
    ...
    OrthoDir segDir(edge e, int segNum)
        {
          OrthoDir od;
          if (segNum > segNumber(e)) THROW(Exception);
...
private:
	...
	BinaryHeap<double, PosInfo*> m_candidateHeap;
...
//intersection test section
    //we have to sort the features and therefore define a special method
      ...
      class FeatureComparer : public Comparer< FeatureLink >
      {
    	public:
        //use both dimension coordinates
		FeatureComparer() 
        {
		}
        //we sort from lower (bottom side) to upper and from left to right side
		int compare(const FeatureLink &f1, const FeatureLink &f2) {
